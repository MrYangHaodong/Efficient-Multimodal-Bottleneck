#!/usr/bin/env bash
# Retrain all 9 trainable methods on DaliaHAR at T=256 (SAX word_length=1),
# 3 folds + aggregate, cross-subject. Same per-method recipe as the T=128 runs,
# only + --word_length 1 (crossattn also uses --sparse_attn_variant orig).
# Results -> results_3p3/T256/<method>/ (T=128 results untouched).
# Usage:  GPUS="0 1 2 3" bash run_daliahar_T256.sh
set -u
source /home/egg8711/miniconda3/etc/profile.d/conda.sh
conda activate maestro || { echo "FATAL: maestro env"; exit 1; }
echo "python -> $(which python)"
HERE="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
TRAIN="$HERE/train"; RES="$HERE/results_3p3/T256"; LOGS="$RES/logs"; mkdir -p "$LOGS"
...
main_of() { case "$1" in
  seqA) echo "main_v6_daliahar_seqfusion_late_fusion.py";;
  crossattn) echo "main_crossattn_v6_daliahar.py";;
  multimodn) echo "main_multimodn_daliahar_dyna.py";;
  dymo) echo "main_dynmm_dymo_daliahar_late_fusion.py";;
  dymo_proto) echo "main_dymo_proto.py";;
  decalign) echo "main_decalign_daliahar_dyna.py";;
  shaspec) echo "main_shaspec_daliahar_dyna.py";;
  qmf|pdf) echo "main_qmfpdf_daliahar_late_fusion.py";;
esac; }
# per-method extra args (recipe), EXCLUDING fold/cuda/num_epochs/results_dir/exp_name
extra_of() { case "$1" in
  ...
  dymo_proto) echo "--dataset daliahar --word_length 1";;
  decalign) echo "--transform sax --word_length 1 --batch_size 128";;  # M=5 + T=256: bs=128 probed ~26GB
...
run_group() {  # $1=method $2=gpu
  ...
  echo "[$(date +%H:%M:%S)] START $m (T=256) on cuda:$gpu -> $log"
  {
    echo "### $m / daliahar T=256 (cuda:$gpu) $(date)"
    for f in 0 1 2; do
