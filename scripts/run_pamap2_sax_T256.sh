#!/usr/bin/env bash
# PAMAP2 at T=256 (word_length=2 SAX, genuine downsample, no interpolation) with
# CLEAN sax, cross-subject, 3 folds, all 5 methods. GPU-PACKED (repeat GPU ids in
# GPUS to run multiple jobs per GPU -> full memory + util). Results -> results_3p3/pamap2_T256/<method>/.
set -u
...
RES="$HERE/results_3p3/pamap2_T256"; LOG="$RES/logs"; mkdir -p "$LOG"
...
extra_of() { case "$1" in
  seqA) echo "--model_arch=v6 --d_model=64 --nhead=8 --base_factor=10 --batch_size=64 --lr=1e-4 --dropout=0.1 --max_modality_drop=0.4 --use_sparse_attn=False --fusion_mode=sequential --seq_random_order --num_layers_per_modal 3 --num_layers 3 --transform sax --word_length 2 --dropout_signal uniform";;
  crossattn) echo "--num_layers_per_modal 3 --num_layers 3 --d_model 64 --sparse_attn_variant orig --transform sax --word_length 2 --dropout_signal uniform";;
  multimodn) echo "--num_layers 3 --num_layers_fus 3 --d_model 64 --word_length 2 --dropout_signal uniform";;
  shaspec) echo "--d_model 64 --num_layers 3 --word_length 2 --dropout_signal uniform";;
  decalign) echo "--transform sax --num_layers 3 --batch_size 64 --word_length 2 --dropout_signal uniform";;
esac; }
resdir() { echo "$RES/$1"; }
...
JOBS=()
for m in "${METHODS[@]}"; do for f in ${FOLDS:-0 1 2}; do
  fold_done "$m" "$f" && { echo "[skip] $m f$f"; continue; }
  JOBS+=("$m|$f"); done; done
echo "pamap2 sax T256 jobs: ${#JOBS[@]} -> ${JOBS[*]} | workers=${#GPU_ARR[@]}"
...
run_one() {  # $1=method $2=fold $3=gpu
  ...
  ( cd "$TRAIN" && python "$s" $ex --fold "$f" --cuda_pick "cuda:$g" \
      --num_epochs 100 --results_dir "$(resdir "$m")" --exp_name "${m}_pamap2_sax_T256" ) >> "$lg" 2>&1
...
wait; rm -f "$QF" "$QL"
echo "== PAMAP2 sax T256 ALL DONE $(date) =="
