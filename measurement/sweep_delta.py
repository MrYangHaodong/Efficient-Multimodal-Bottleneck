#!/usr/bin/env python3
"""Delta sweep — trace the seqA+RL accuracy/compute frontier on a fixed policy.

OURS_DELTA is the test-time stop bias: the walk stops when
    Q[STOP] + delta >= max Q[acquire]
Raising delta stops earlier (fewer modalities, lower latency, usually lower F1);
lowering it acquires more.  It is a DEPLOYMENT lever on an already-trained net —
no retraining — so one policy traces its whole frontier.

This picks the operating point per dataset.  UTD-MHAD's published delta=0.0 came
from this sweep: F1 rose 0.869 -> 0.914 for +0.17 modalities and ~1.2ms.

SELF-CONTAINED: inlines the whole harness; only `models/` is imported.
Models are loaded ONCE per fold and the walk re-run for each delta, so the sweep
costs one model load, not one per delta.
...
#  Main
# ══════════════════════════════════════════════════════════════════════════════
DEFAULT_DELTAS = [0.15, 0.10, 0.05, 0.00, -0.05, -0.10, -0.15]
...
def main():
    ...
    ap.add_argument('--variant', choices=['a0', 'c2'], default='c2')
    ...
    ap.add_argument('--out', default=os.path.join(HERE, 'results_delta_sweep.json'))
    a = ap.parse_args()

    dev = torch.device('cpu') if a.cpu or not torch.cuda.is_available() \
          else torch.device(f'cuda:{a.gpu}')

    print(f'\n=== Delta sweep ({a.variant.upper()} walk)  device={dev} ===')
    print(f'deltas: {a.deltas}')
    print(f'n/fold={a.n or "all"}  warmup={a.warmup}\n')
    ...
    for ds in a.datasets:
        ...
        D = DsCfg(ds)
        # delta -> accumulators across folds
        acc = {d: dict(f1s=[], lats=[], mods=[]) for d in a.deltas}
        print(f'── {DISPLAY[ds]}  (M={D.M}, {dcfg["folds"]} folds, published δ={dcfg["delta"]}) ──')

        for fold in range(dcfg['folds']):
            batches, mods, _, _, _ = load_real_test(ds, fold, dev)
            samples, labels = make_samples(batches, mods, a.n)

            # Load once, reuse for every delta.
            ...
            if a.variant == 'c2':
                ...
                walk = lambda s: walk_c2(s, seqa, enc_s, head, net, vv, cfg, mods, dev)
            else:
                steps = {m: SeqAStep(seqa.branches[m]).eval().to(dev) for m in mods}
                walk = lambda s: walk_a0(s, seqa, steps, head, net, vv, cfg, mods, dev)

            print(f'  fold{fold}  n={len(samples)}')
            for d in a.deltas:
                OURS_DELTA = d                      # walk_* reads this module global
                f1, lats, mdp = _timed(walk, samples, labels, dev, a.warmup)
                acc[d]['f1s'].append(f1); acc[d]['lats'].append(lats.mean())
                acc[d]['mods'].append(mdp)
                print(f'    δ={d:+.2f}  F1={f1:.4f}  lat={lats.mean():6.2f}ms  '
                      f'mods={mdp:.2f}/{D.M}')
        ...
        results[ds] = {f'{d:+.2f}': dict(
            delta=d, f1=float(np.mean(v['f1s'])), lat=float(np.mean(v['lats'])),
            mods=float(np.mean(v['mods'])), M=D.M) for d, v in acc.items()}

        print(f'\n  {DISPLAY[ds]} summary (mean over folds):')
        print(f'    {"delta":>7} {"F1":>8} {"lat(ms)":>9} {"mods":>12}')
        print(f'    {"-"*7} {"-"*8} {"-"*9} {"-"*12}')
        for d in a.deltas:
            r = results[ds][f'{d:+.2f}']
            star = '  ← published' if abs(d - dcfg['delta']) < 1e-9 else ''
            print(f'    {d:+7.2f} {r["f1"]:8.4f} {r["lat"]:9.2f} '
                  f'{r["mods"]:6.2f}/{r["M"]:<5}{star}')
        print()
