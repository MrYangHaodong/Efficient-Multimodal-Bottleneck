#!/usr/bin/env python3
"""GPU latency + F1 benchmark — seqA+RL vs AdaMML / DynMM / DyMo, 7 datasets.
...
Timing model — data is PRE-LOADED to GPU before the timer starts.  Each timed
call is pure model compute; no disk read and no host->device transfer is
included.  (For loading-inclusive numbers see bench_loading.py.)
...
Usage:
  cd runs/measurement
  CUDA_VISIBLE_DEVICES=0 python bench_gpu.py [--n 50] [--warmup 5] [--variant a0|c2|both]
"""
...
def main():
    ...
    ap.add_argument('--variant', choices=['a0', 'c2', 'both'], default='a0',
                    help='seqA+RL walk variant to time')
    ap.add_argument('--datasets', nargs='+', default=list(DS_CFG), choices=list(DS_CFG))
    ap.add_argument('--out', default=os.path.join(HERE, 'results_gpu.json'))
    a = ap.parse_args()

    dev = torch.device(f'cuda:{a.gpu}' if torch.cuda.is_available() else 'cpu')
    if dev.type != 'cuda':
        print('WARNING: CUDA unavailable — running on CPU (use bench_cpu.py instead)')

    want_a0 = a.variant in ('a0', 'both')
    want_c2 = a.variant in ('c2', 'both')

    print(f'\n=== GPU benchmark (compute only, data pre-loaded)  device={dev} ===')
    print(f'n/fold={a.n or "all"}  warmup={a.warmup}  variant={a.variant}\n')
