...
class AdaMMLNet(nn.Module):
    ...
    @torch.no_grad()
    def forward_selective(self, xd):
        """Batched SELECTIVE execution: run encoder m only on the samples whose policy
        kept m (gather -> encode -> scatter).  Numerically identical to forward(mode=
        'policy') — unkept modalities are masked out of the late fusion anyway — but it
        does NOT spend encoder FLOPs on dropped modalities, at any batch size.  Eval only."""
        dec, plogits = self.policy(xd)                                                    # [B,M]
        B = dec.shape[0]
        allzero = dec.sum(1) == 0                                                         # guarantee >=1 kept
        if allzero.any():
            top = plogits[..., 1].argmax(1)
            dec[allzero, top[allzero]] = 1.0
        L = None
        for mi, m in enumerate(self.modalities):
            keep = dec[:, mi] > 0.5                                                       # [B] bool
            if not bool(keep.any()):
                continue
            lm = self.encoders[m](xd[m][keep])[0]                                         # encode only kept sub-batch
            if L is None:
                L = torch.zeros(B, self.M, lm.shape[-1], device=lm.device, dtype=lm.dtype)
            L[keep, mi] = lm
        w = F.softmax(self.lf, 0).view(1, self.M, 1)
        d = dec.unsqueeze(-1)
        out = (w * d * L).sum(1) / (w * d).sum(1).clamp_min(1e-6)
        return out, dec, plogits
