...
class ProbSparseAttentionOpt(nn.Module):
    _attention_weights: Optional[torch.Tensor]   # class-body annotation for TorchScript
...
class SequentialModalityEncoder(nn.Module):
    """Drop-in replacement for :class:`BatchedModalityEncoder` that encodes
    modalities **one at a time** instead of all together.

    Same construction args, same stored weights (an identical ``self.layers``
    stack of ``BatchedTransformerBlock`` with M_total param sets), and the same
    ``forward`` signature — so it is a true drop-in and state_dicts are
    interchangeable with the batched encoder (identical
    ``batched_modality_encoder.layers.*`` keys). The only difference is
    scheduling: this variant runs a **single-modality encoder pass per
    modality** (M=1) and re-stacks, so modalities are never processed jointly.

    Because ``BatchedTransformerBlock`` mixes nothing across the modality axis
    (its einsum is per-modality), the output is mathematically identical to the
    batched encoder — this only changes *how* the work is scheduled.
    """
    ...
    def _encode_one_modality(self, x_single: torch.Tensor,
                             mod_idx: torch.Tensor,
                             capture_layer: Optional[int],
                             layer_keep_single: Optional[torch.Tensor]):
        """Single-modality encoder: run ONE modality [1, B, L, D] through the full
        layer stack using its own stored weight slice (``mod_idx``: LongTensor [1]).
        Returns [1, B, L_out, D] (or a (final, captured) tuple if capture_layer)."""
        x = x_single
        captured = None
        for i, layer in enumerate(self.layers):
            x_new = layer(x, mod_idx)
            if layer_keep_single is not None and i >= 1:
                k = layer_keep_single[:, :, i - 1].view(
                    x_new.shape[0], x_new.shape[1], 1, 1).to(x_new.dtype)
                x = x_new * k + x * (1.0 - k)
            else:
                x = x_new
            if capture_layer is not None and i == capture_layer:
    ...
    def forward(self, x_stacked: torch.Tensor,
                ...
                capture_layer: Optional[int] = None,
                layer_keep: Optional[torch.Tensor] = None):
        """Same signature/semantics as BatchedModalityEncoder.forward, but loops
        the single-modality encoder over the M_active modalities one at a time."""
        del factor
        if layer_keep is not None:
            assert not self.use_distill, \
                'layer_keep (ADMN LayerDrop) needs use_distill=False (length-preserving layers)'
        M_active = x_stacked.shape[0]
        outs, caps = [], []
        for m in range(M_active):
            x_single = x_stacked[m:m + 1]                             # [1, B, L, D]
            # A single-modality call MUST name its param slice: mod_indices=None
            # would make BatchedTransformerBlock use ALL M_total stored sets.
            if mod_indices is None:
                mod_idx = torch.tensor([m], dtype=torch.long, device=x_stacked.device)
            else:
                mod_idx = mod_indices[m:m + 1]
            lk_single = None if layer_keep is None else layer_keep[m:m + 1]
            res = self._encode_one_modality(x_single, mod_idx, capture_layer, lk_single)
            if capture_layer is not None:
                outs.append(res[0]); caps.append(res[1])
            else:
                outs.append(res)
        x_out = torch.cat(outs, dim=0)                               # [M_active, B, L_out, D]
        if capture_layer is not None:
            return x_out, torch.cat(caps, dim=0)
        return x_out
...
class SimpleMBTFusionAdaptiveMLPDownsampleBmm(nn.Module):
    ...
    def __init__(self, input_dims: Dict[str, int],
        ...
        assert seq_agg_mode in ('gate', 'mean', 'mlp'), \
            f"seq_agg_mode must be 'gate', 'mean', or 'mlp', got '{seq_agg_mode}'"
        self.seq_agg_mode = seq_agg_mode
        if fusion_mode == 'sequential' and use_bottleneck:
            n_fuse = num_layers - fusion_layer
            # GRU update gates — only needed when seq_agg_mode='gate'.
            # seq_agg_mode='mean' uses a parameter-free running average instead.
            # seq_agg_mode='mlp' uses a 2-layer MLP: Linear(2D,D)->GELU->Linear(D,D).
    ...
    def _fuse_sequential(self, x, bottleneck_init, mod_indices, M_active,
        ...
        if getattr(self, '_collect_snapshots', False):
            self._bn_snapshots = []   # per-step: {lyr: (B,K,H)} AFTER that step
            self._tok_snapshots = []  # per-step: (B,L,H) token output AFTER that step
        ...
        for step, mi in enumerate(order):
            ...
            for lyr in fusion_layers:
                ...
                if step == 0:
                    ...
                    if self.seq_agg_mode == 'mean':
                        # Running average: equal weight to every modality seen so far.
                        # After step k, bn_state = (1/(k+1)) * sum(bn_0..bn_k).
                        bn_state[lyr] = (step * bn_state[lyr] + bn_new) / (step + 1)
                    elif self.seq_agg_mode == 'mlp':
                        # 2-layer MLP: directly maps [h_old; h_new] -> h_next.
                        h_cat = torch.cat([bn_state[lyr], bn_new], dim=-1)
                        bn_state[lyr] = self.seq_mlps[fuse_idx](h_cat)
                    else:
            ...
            # Selector oracle snapshots (off by default; enabled by setting
            # fusion._collect_snapshots = True before the forward call).
            if getattr(self, '_collect_snapshots', False):
                self._bn_snapshots.append(
                    {lyr: bn_state[lyr].detach().clone()
                     for lyr in fusion_layers if bn_state[lyr] is not None}
                )
                self._tok_snapshots.append(xm[0].detach())     # (B, L_out, H)
    ...
    def fuse_one_modality(self, xm, mod_idx, step_k, bn_state, bottleneck_init):
        """Incremental one-step sequential fusion for selector oracle computation.

        Replicates exactly one iteration of ``_fuse_sequential``'s inner loop for
        a single modality, reading from / updating ``bn_state`` in the same way.
        No gradient is intended here — use inside a ``torch.no_grad()`` block.

        Args:
            xm:             (B, L, H) – pre-encoded modality tokens (output of
                             ``DualVideoBottleneckModelV6Downsample.prepare_for_selector``)
            mod_idx:        int        – canonical modality index for block dispatch
            step_k:         int        – 0-based step index (0 = first modality)
            bn_state:       dict {lyr: (B, K, H)} | None — accumulated bottleneck
                             state; None is valid only when step_k == 0
            bottleneck_init: (B, K, H) – initial bottleneck (only used at step_k == 0)

        Returns:
            out_tok:      (B, L_out, H)       – output tokens (not yet final-normed)
            bn_state_new: dict {lyr: (B, K, H)} – updated accumulated state
        """
        fusion_layers = list(range(self.fusion_layer, self.num_layers))
        mod_idx_t = torch.tensor([mod_idx], dtype=torch.long, device=xm.device)
        xm_ = xm.unsqueeze(0)   # (1, B, L, H)
        bn_state_new = {}
        ...
        if step_k == 0:
            ...
            bn_depth = None  # unused for seq_depth_flow=False + step_k>0
        ...
        for lyr in fusion_layers:
            ...
            if not self.bottleneck_head_pos:
                combined = torch.cat([xm_, bn_stack], dim=2)
            else:
                combined = torch.cat([bn_stack, xm_], dim=2)

            out = block(combined, mod_idx_t)       # (1, B, L_out, H)
            L_out = out.shape[2]
            ...
            if step_k == 0:
                bn_state_new[lyr] = bn_new
                bn_depth = bn_new                  # depth-carry for step 0
            else:
                if self.seq_agg_mode == 'mean':
                    bn_state_new[lyr] = (step_k * bn_state[lyr] + bn_new) / (step_k + 1)
                elif self.seq_agg_mode == 'mlp':
                    h_cat = torch.cat([bn_state[lyr], bn_new], dim=-1)
                    bn_state_new[lyr] = self.seq_mlps[fuse_idx](h_cat)
                else:
                    bn_state_new[lyr] = self._seq_gate_update(fuse_idx, bn_state[lyr], bn_new)
                if self.seq_depth_flow:
                    bn_depth = bn_new

        return xm_[0], bn_state_new               # (B, L_out, H), {lyr: (B, K, H)}
...
class DualVideoBottleneckModelV6Downsample(nn.Module):
    ...
    def __init__(self, cfg, output_dim=1, input_length=45, d_model=384, nhead=16,
        ...
        # The bottleneck *fusion* is always the batched (bmm) implementation.
        # `use_batched_fusion` selects the per-modality *encoder*:
        #   True  -> BatchedModalityEncoder   (all modalities per grouped-GEMM)
        #   False -> SequentialModalityEncoder (one modality at a time)
        # Both hold identical weights and give identical results (the block mixes
        # nothing across the modality axis); only the scheduling differs.
        ...
        # ---- Per-modality encoder (replaces parent's per-modal dict).
        # Batched = all modalities in one grouped-GEMM per layer (default, fast).
        # Sequential = one modality at a time (identical weights & result).
        # Attribute name kept as `batched_modality_encoder` either way so the
        # forward call sites and checkpoint keys are unchanged.
        _EncCls = BatchedModalityEncoder if use_batched_fusion else SequentialModalityEncoder
        self.batched_modality_encoder = _EncCls(
    ...
    # ------------------------------------------------------------------
    # Selector oracle helper
    # ------------------------------------------------------------------
    def prepare_for_selector(self, high_dim_inputs, low_dim_inputs, factor=None):
        """Encode all modalities and compute the initial bottleneck for oracle.

        Call inside ``torch.no_grad()`` during selector oracle computation.

        Returns:
            encoded:         dict {m: (B, L, H)} – per-modality encoder outputs
            bottleneck_init: (B, K, H)            – initial bottleneck token set
            modalities:      list[str]             – canonical modality order
        """
        base_factor = factor or self.factor
        ...
        encoded = self._encode_modalities(projected_features, None, base_factor)

        fusion = self.bottleneck_fusion
        batch_size = next(iter(high_dim_inputs.values())).shape[0]
        if fusion.bottleneck_init_mode == 'modal_mean_sinpe':
            x_list = [encoded[m] for m in self.modalities if m in encoded]
            modal_mean = torch.stack(x_list, dim=0).mean(dim=(0, 2))  # (B, H)
            bottleneck_init = (modal_mean.unsqueeze(1)
                               .expand(-1, fusion.n_bottlenecks, -1)
                               + fusion.bottleneck_pe).contiguous()   # (B, K, H)
        else:
            bottleneck_init = fusion.bottleneck.expand(
                batch_size, -1, -1).contiguous()                       # (B, K, H)

        return encoded, bottleneck_init, self.modalities
