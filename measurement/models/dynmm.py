...
class MBTransformer(nn.Module):
    """MultiBench `Transformer`: 1x1 conv projection + N-layer TransformerEncoder, last-token pooled."""
    def __init__(self, n_features: int, dim: int, nhead: int = 8, num_layers: int = 6):
        super().__init__()
...
class OrigDynMMExpertG(nn.Module):
    ...
    def __init__(self, sub_modalities, full_modalities, variates, num_classes, input_mode='float',
                 embed_dim=128, nhead=8, num_layers=6, alphabet_size=20, token_dim=16, hidden=128, dropout=0.1):
        super().__init__()
        ...
        self.encoders = nn.ModuleDict(
            {m: MBTransformer(in_dim(m), embed_dim, nhead=nhead, num_layers=num_layers) for m in self.sub})
...
class OrigGateG(nn.Module):
    ...
    def __init__(self, full_modalities, variates, n_branch, input_mode='float',
                 gate_dim=16, nhead=8, num_layers=6, alphabet_size=20, token_dim=16):
        super().__init__()
        ...
        self.tr = MBTransformer(gin, gate_dim, nhead=nhead, num_layers=num_layers)
        self.fc = nn.Linear(gate_dim, n_branch)
...
class OrigDynMMBranchG(nn.Module):
    """N frozen experts + a per-sample DiffSoftmax gate with a resource reg (NO distillation).

    resource_mode:
      'expensive' : FAITHFUL original ModalityDynMM (DynMM-b) — penalize the mean gate weight
                    routed to the most expensive (full-fusion) branch, i.e. weight[:, argmax_flop].mean()
                    (matches imdb_dyn.py / affect_dyn.py `weight[:, 1].mean()`).
      'flop'      : FLOP-weighted expected compute sum_b w_b * flop_b (the fusion-level DynMM form).
    """
    def __init__(self, experts, gate, flop_vec, tau=1.0, hard=True, resource_mode='expensive'):
        super().__init__()
        ...
        self.expensive_idx = int(torch.tensor(flop_vec).argmax())
    ...
    def forward(self, x, additional_loss=False):
        ...
        if additional_loss:
            if self.resource_mode == 'flop':
                reg = (w * self.flop).sum(1).mean()                            # expected FLOPs
            else:
                reg = w[:, self.expensive_idx].mean()                         # faithful: weight on expensive branch
            return out, reg
        return out, w


# Backward-compat alias (old 3-branch FLOP-weighted call sites).
OrigDynMM3BranchG = OrigDynMMBranchG
