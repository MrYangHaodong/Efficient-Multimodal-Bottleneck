"""MAESTRO baseline (`CrossAttnTransformerClf`) — VENDORED for the energy_adaptive bundle.

Verbatim extraction of the classes `CrossAttnTransformerClf` needs from
`MAESTRO/models/our_models.py` (the module `training/maestro.py` and
`runs/robustness_missingness/run_robustness.py` import).  Only the unused
`InformerClf`, the torchvision / tensorboard / pandas imports and the
`from utils import *` wildcard were dropped — `CrossAttnTransformerClf` and its
dependencies use none of them, so every layer, buffer and parameter NAME is
unchanged and MAESTRO checkpoints load with strict=True.

Kept (in source order, unmodified bodies):
  ProbSparseAttention, SparseMoEFeedForward, ModalityPositionalEncoder,
  TemporalPositionalEncoder, InformerEncoderLayerWithMoE, modality_dropout,
  InformerEncoderLayer, CrossAttnTransformerClf

forward(x, modality_dropout_prob=0.0, training=False) -> (logits, dynamic_factor)
with x = (B, T, sum(variates)) in `self.modalities` order.
"""
import math
import numpy as np
import torch
