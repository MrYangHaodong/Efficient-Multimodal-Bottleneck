#!/usr/bin/env python3
"""CPU latency + F1 benchmark — seqA+RL vs AdaMML / DynMM / DyMo, 7 datasets.

SELF-CONTAINED: this file inlines the whole measurement harness (dataset config,
model loaders, RL policy, stacked-BMM encoder, walk variants, baseline forwards).
The only local import is the `models/` package, which holds the trained
architectures and is too large to inline.

Timing model — data is PRE-LOADED into RAM before the timer starts.  Each timed
call is pure model compute; no disk read is included.  (For loading-inclusive
numbers see bench_loading.py.)

CPU latency is thread-count sensitive.  --threads pins torch's intra-op pool so
runs are comparable; the default leaves PyTorch's own choice in place and the
value actually used is printed in the header and the summary.

Walk variants:
  A0  encode+fuse each acquired modality sequentially inside the RL loop
  C2  stacked-BMM pre-encode all M modalities upfront, then a fuse-only walk

Operating points (match the comparison CSV):
  delta=0.1 everywhere except UTD-MHAD which uses delta=0.0
  MM-Fi    : rebalanced checkpoint  + rl_policy_net_mmfi_rebal/
  UTD-MHAD : dropaug checkpoint     + rl_policy_net_utd_mhad_dropaug/

Usage:
  cd runs/measurement
  CUDA_VISIBLE_DEVICES="" python bench_cpu.py [--n 50] [--threads 8] [--variant c2]
"""
...
# ══════════════════════════════════════════════════════════════════════════════
#  Timed loops — data already resident in RAM
# ══════════════════════════════════════════════════════════════════════════════
def _sync(dev):
    if dev.type == 'cuda':
...
def main():
    ...
    ap.add_argument('--threads', type=int, default=0,
                    help='torch intra-op CPU threads (0 = PyTorch default)')
    ap.add_argument('--variant', choices=['a0', 'c2', 'both'], default='c2',
                    help='seqA+RL walk variant to time')
    ap.add_argument('--datasets', nargs='+', default=list(DS_CFG), choices=list(DS_CFG))
    ap.add_argument('--out', default=os.path.join(HERE, 'results_cpu.json'))
    a = ap.parse_args()
    ...
    want_a0 = a.variant in ('a0', 'both')
    want_c2 = a.variant in ('c2', 'both')

    print(f'\n=== CPU benchmark (compute only, data pre-loaded)  device={dev} ===')
    print(f'threads={torch.get_num_threads()}  n/fold={a.n or "all"}  '
          f'warmup={a.warmup}  variant={a.variant}\n')
    ...
    for ds in a.datasets:
        ...
        keys = (['a0'] if want_a0 else []) + (['c2'] if want_c2 else []) + ['adamml']
        ...
        for fold in range(dcfg['folds']):
            batches, mods, variates, C, _ = load_real_test(ds, fold, dev)
            ...
            if want_a0:
                ...
                f1, lats, mdp = _timed(
                    lambda s: walk_a0(s, seqa, steps, head, net, vv, cfg, mods, dev),
                    samples, labels, dev, a.warmup)
                acc['a0']['f1s'].append(f1); acc['a0']['lats'].append(lats.mean())
                acc['a0']['mods'].append(mdp)
                print(f'    A0      F1={f1:.4f}  lat={lats.mean():.2f}ms  mods={mdp:.2f}/{D.M}')
                del steps
            ...
            if want_c2:
                ...
                f1, lats, mdp = _timed(
                    ...
                    samples, labels, dev, a.warmup)
                acc['c2']['f1s'].append(f1); acc['c2']['lats'].append(lats.mean())
                acc['c2']['mods'].append(mdp)
                print(f'    C2      F1={f1:.4f}  lat={lats.mean():.2f}ms  mods={mdp:.2f}/{D.M}')
                del enc_s
            ...
            f1, lats, mdp = _timed(
                lambda s: (lambda o: (o[0], len(o[1][1])))(fwd_adamml(adamml, s, mods)),
                samples, labels, dev, a.warmup)
            acc['adamml']['f1s'].append(f1); acc['adamml']['lats'].append(lats.mean())
            acc['adamml']['mods'].append(mdp)
            print(f'    AdaMML  F1={f1:.4f}  lat={lats.mean():.2f}ms  mods={mdp:.2f}/{D.M}')
            del adamml
            ...
            if dcfg['has_dynmm']:
                try:
                    ...
                    f1, lats, mdp = _timed(_fwd_dyn, samples, labels, dev, a.warmup)
                    acc['dynmm']['f1s'].append(f1); acc['dynmm']['lats'].append(lats.mean())
                    acc['dynmm']['mods'].append(mdp)
                    print(f'    DynMM   F1={f1:.4f}  lat={lats.mean():.2f}ms  mods={mdp:.2f}/{D.M}')
                    del gate, experts
            ...
            if dcfg['has_dymo']:
                try:
                    ...
                    f1, lats, mdp = _timed(_fwd_dymo, samples, labels, dev, a.warmup)
                    acc['dymo']['f1s'].append(f1); acc['dymo']['lats'].append(lats.mean())
                    acc['dymo']['mods'].append(mdp)
                    print(f'    DyMo    F1={f1:.4f}  lat={lats.mean():.2f}ms  mods={mdp:.2f}/{D.M}')
                    del proto
                except Exception as e:
                    print(f'    DyMo    SKIP: {e}')
                    for k in ('f1s', 'lats', 'mods'):
    ...
    # ── Summary ───────────────────────────────────────────────────────────────
    MODELS = [k for k in ['a0', 'c2', 'adamml', 'dynmm', 'dymo']
              if any(k in results[ds] for ds in results)]
    NAMES = {'a0': 'seqA+RL(A0)', 'c2': 'seqA+RL(C2)', 'adamml': 'AdaMML',
             'dynmm': 'DynMM', 'dymo': 'DyMo'}

    print(f'\n{"":14}' + ''.join(f'  {NAMES[m]:>16}' for m in MODELS))
    print(f'{"Dataset":<14}' + ''.join(f'  {"lat(ms)/F1":>16}' for m in MODELS))
    ...
    for ds in results:
        ...
        for m in MODELS:
            ...
            if r is None or np.isnan(r['lat']):
                ...
                print(f'  {r["lat"]:7.2f}ms/{r["f1"]:.3f}', end='')
        print()

    ours = 'c2' if want_c2 else 'a0'
    print(f'\nSpeedup {NAMES[ours]} vs baseline  (lat_baseline / lat_ours):')
    for bm in [m for m in MODELS if m not in ('a0', 'c2')]:
        spds = [results[ds][bm]['lat'] / results[ds][ours]['lat']
                for ds in results
                if bm in results[ds] and not np.isnan(results[ds][bm]['lat'])]
        if spds:
            print(f'  vs {NAMES[bm]:<12}: {np.exp(np.mean(np.log(spds))):.3f}× geomean  '
