#!/usr/bin/env python3
"""fp16 + entropy early-exit study — does either buy anything at batch size 1?

Compares six inference modes on the same samples:
  C2 fp32              stacked-BMM pre-encode walk, float32 (reference)
  C2 fp16              same walk with weights and activations cast to half
  C2 fp16 + EE 0.95    plus an entropy early exit at max-softmax > 0.95
  C2 fp16 + EE 0.90    plus an entropy early exit at max-softmax > 0.90
  AdaMML fp32          baseline
  AdaMML fp16          baseline, half precision

Expect fp16 to be a WASH or slightly slower here: at B=1 with D=128 and K=16
bottleneck tokens the matrices are far too small to engage tensor cores, so
kernel-launch overhead dominates and the extra casts cost more than the math
saves.  Expect the early exit to rarely fire at delta<=0.0, because the RL stop
criterion already triggers before confidence reaches those thresholds — the
printed depths show whether it fired at all.

SELF-CONTAINED: inlines the whole harness; only `models/` is imported.
...
# ══════════════════════════════════════════════════════════════════════════════
#  Test data (pre-loaded to device — NOT part of any timed region)
# ══════════════════════════════════════════════════════════════════════════════
...
# ══════════════════════════════════════════════════════════════════════════════
#  Timed loops — data already resident on device
# ══════════════════════════════════════════════════════════════════════════════
def _sync(dev):
    if dev.type == 'cuda':
...
#  Main
# ══════════════════════════════════════════════════════════════════════════════
def cast_sample(s, dtype):
    """Cast every modality tensor in a sample dict to `dtype`."""
    return {m: v.to(dtype) for m, v in s.items()}


def main():
    global OURS_DELTA
    ap = argparse.ArgumentParser()
    ap.add_argument('--n',      type=int, default=200, help='samples per fold (0=all)')
    ap.add_argument('--warmup', type=int, default=5)
    ap.add_argument('--gpu',    type=int, default=0)
    ap.add_argument('--ee',     type=float, nargs='+', default=[0.95, 0.90],
                    help='entropy early-exit confidence thresholds')
    ap.add_argument('--datasets', nargs='+', default=['utd_mhad'], choices=list(DS_CFG))
    ap.add_argument('--out', default=os.path.join(HERE, 'results_fp16_ee.json'))
    a = ap.parse_args()

    if not torch.cuda.is_available():
        print('ERROR: fp16 study needs CUDA.'); sys.exit(1)
    dev = torch.device(f'cuda:{a.gpu}')

    print(f'\n=== fp16 + entropy early-exit study  device={dev} ===')
    print(f'n/fold={a.n or "all"}  warmup={a.warmup}  EE thresholds={a.ee}\n')
    ...
    for ds in a.datasets:
        ...
        modes = ['c2_fp32', 'c2_fp16'] + [f'c2_fp16_ee{t}' for t in a.ee] \
                + ['am_fp32', 'am_fp16']
        acc = {k: dict(f1s=[], lats=[], mods=[]) for k in modes}

        for fold in range(dcfg['folds']):
            batches, mods, _, _, _ = load_real_test(ds, fold, dev)
            samples, labels = make_samples(batches, mods, a.n)
            s16 = [cast_sample(s, torch.float16) for s in samples]
            print(f'  fold{fold}  n={len(samples)}')
            ...
            enc_s = build_stacked_encoder(seqa, mods, dev, verbose=(fold == 0))
            f1, lats, mdp = _timed(
                lambda s: walk_c2(s, seqa, enc_s, head, net, vv, cfg, mods, dev),
                samples, labels, dev, a.warmup)
            acc['c2_fp32']['f1s'].append(f1); acc['c2_fp32']['lats'].append(lats.mean())
            acc['c2_fp32']['mods'].append(mdp)
            print(f'    C2 fp32        F1={f1:.4f}  lat={lats.mean():6.2f}ms  mods={mdp:.2f}/{D.M}')
            del seqa, head, enc_s
            torch.cuda.empty_cache()

            # ── C2 fp16 (+ optional early exit) ────────────────────────────
            seqa16 = load_seqa(ds, fold, dev).half()
            head16 = SeqAHead(seqa16).eval().to(dev)
            enc16 = build_stacked_encoder(seqa16, mods, dev, verbose=(fold == 0))

            for tag, thr in [('c2_fp16', 0.0)] + [(f'c2_fp16_ee{t}', t) for t in a.ee]:
                f1, lats, mdp = _timed(
                    lambda s, thr=thr: walk_c2(s, seqa16, enc16, head16, net, vv, cfg,
                                               mods, dev, conf_thresh=thr),
                    s16, labels, dev, a.warmup)
                acc[tag]['f1s'].append(f1); acc[tag]['lats'].append(lats.mean())
                acc[tag]['mods'].append(mdp)
                label = 'C2 fp16' if thr == 0.0 else f'C2 fp16+EE{thr}'
                print(f'    {label:<14} F1={f1:.4f}  lat={lats.mean():6.2f}ms  mods={mdp:.2f}/{D.M}')
            del seqa16, head16, enc16, net
            torch.cuda.empty_cache()

            # ── AdaMML fp32 / fp16 ─────────────────────────────────────────
            for tag, dtype, smp in [('am_fp32', torch.float32, samples),
                                    ('am_fp16', torch.float16, s16)]:
                am = load_adamml_g00(ds, fold, dev)
                if dtype == torch.float16:
                    am = am.half()
                f1, lats, mdp = _timed(
                    lambda s: (lambda o: (o[0], len(o[1][1])))(fwd_adamml(am, s, mods)),
                    smp, labels, dev, a.warmup)
                acc[tag]['f1s'].append(f1); acc[tag]['lats'].append(lats.mean())
                acc[tag]['mods'].append(mdp)
                label = 'AdaMML fp32' if dtype == torch.float32 else 'AdaMML fp16'
                print(f'    {label:<14} F1={f1:.4f}  lat={lats.mean():6.2f}ms  mods={mdp:.2f}/{D.M}')
                del am
                torch.cuda.empty_cache()

        results[ds] = {k: dict(
            f1=float(np.mean(v['f1s'])), lat=float(np.mean(v['lats'])),
            mods=float(np.mean(v['mods'])), M=D.M) for k, v in acc.items()}

        # ── Per-dataset summary ───────────────────────────────────────────
        NAMES = {'c2_fp32': 'C2 fp32', 'c2_fp16': 'C2 fp16',
                 'am_fp32': 'AdaMML fp32', 'am_fp16': 'AdaMML fp16'}
        for t in a.ee:
            NAMES[f'c2_fp16_ee{t}'] = f'C2 fp16+EE{t}'
        base = results[ds]['c2_fp32']['lat']
        print(f'\n  {DISPLAY[ds]} summary (mean over folds):')
        print(f'    {"mode":<16} {"F1":>8} {"lat(ms)":>9} {"vs C2 fp32":>11} {"mods":>12}')
        print(f'    {"-"*16} {"-"*8} {"-"*9} {"-"*11} {"-"*12}')
        for k in modes:
            r = results[ds][k]
            print(f'    {NAMES[k]:<16} {r["f1"]:8.4f} {r["lat"]:9.2f} '
                  f'{base / r["lat"]:10.3f}× {r["mods"]:6.2f}/{r["M"]:<5}')

        ee_keys = [f'c2_fp16_ee{t}' for t in a.ee]
        if ee_keys:
            fired = any(results[ds][k]['mods'] < results[ds]['c2_fp16']['mods'] - 1e-6
                        for k in ee_keys)
            print(f'    early exit fired: {"YES" if fired else "NO — RL stop always came first"}')
        print()
