...
crossattn/multimodn/shaspec/decalign (+dymo) baselines on CMU-MOSEI at T=50,
FLOAT input (no SAX). Same method as measure_baselines_gflops_dsads.py.
Output -> results_3p3/baselines_gflops_mosei.json.
Run from clean_models/train/:  python ../results_3p3/measure_baselines_gflops_mosei.py
"""
...
RESROOT = os.path.join(_CM, 'train', 'results_mosei_3seed')
...
def cfg_of(cj):
    ...
    return _Cfg(mods, {m: FEATURE_DIMS[m] for m in mods}, cj.get('num_classes') or 2)


def run_dir(method):
    pats = {
        'crossattn': '2026-*_CMU_MOSEI_crossattn_fixed_acc2_s42',
        ...
        'dymo':      '2026-*_mosei_dymo_acc2_s42',
    }
    ds = sorted(glob.glob(os.path.join(RESROOT, pats[method])))
...
def build_fwd(method, cj, cfg, C):
    x = torch.randn(1, T, C).to(DEV)                       # MOSEI: float input
    if method == 'crossattn':
...
print(f"{'method':10s} {'lib':>9s} {'TRUE(T50)':>11s}")
out = {}
for method in ['crossattn', 'multimodn', 'decalign', 'shaspec', 'dymo']:
    ...
    try:
        ...
        out[method] = {'mosei': {'lib_gflops': lib, 'true_gflops': true}}
        print(f"{method:10s} {lib:9.4f} {true:11.4f}")
    except Exception as e:
        print(f"{method:10s}  ERROR: {e}")
F.scaled_dot_product_attention = _orig
json.dump(out, open(os.path.join(_HERE, 'baselines_gflops_mosei.json'), 'w'), indent=2)
print('saved -> results_3p3/baselines_gflops_mosei.json')
