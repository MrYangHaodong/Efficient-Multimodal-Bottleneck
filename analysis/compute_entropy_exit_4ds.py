...
early-exit method as the gap-head exit scripts, but head_mode='entropy' and reading
the results_3p3/entropy_head/<ds>/seqA models). Output -> entropy_head/exit_entropy_4ds.json
...
ENT = os.path.join(_ROOT, 'results_3p3', 'entropy_head')
...
def make_build(C, NUMC, T, vhigh, vlow, modalities_cfg):
    ...
    def build(mods):
        return DualVideoBottleneckModelV6Downsample(
            head_mode='entropy', cfg=_VC(mods), output_dim=NUMC, input_length=T, d_model=C['d_model'], nhead=C['nhead'],
...
def run_dataset(ds):
    seqa_dir = sorted(glob.glob(os.path.join(ENT, ds, 'seqA', '*')))[0]
...
json.dump(out, open(os.path.join(ENT, 'exit_entropy_4ds.json'), 'w'), indent=2)
print('saved -> results_3p3/entropy_head/exit_entropy_4ds.json')
