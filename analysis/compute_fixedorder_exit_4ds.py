"""FIXED-val-LOO-order-trained seqA adaptive-exit curves on 4 datasets (same val-LOO + entropy
...
def run_dataset(ds):
    seqa_dir = sorted(glob.glob(os.path.join(_ROOT, 'results_3p3', 'seqA_fixedorder', ds, 'seqA', '*')))[0]
    C = json.load(open(os.path.join(seqa_dir, 'config.json')))
    # eval: respect the fed prefix order (override the trained fixed order)
    C['seq_modality_order'] = None; C['seq_random_order'] = False
    if ds == 'iemocap':
...
json.dump(out, open(os.path.join(_ROOT, 'results_3p3', 'seqA_fixedorder_exit_4ds.json'), 'w'), indent=2)
print('saved -> results_3p3/seqA_fixedorder_exit_4ds.json')
