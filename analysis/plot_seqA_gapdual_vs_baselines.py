#!/usr/bin/env python
"""Same figure as gflops_acc_f1_seqA_vs_baselines_3ds.png but the V6-seqA adaptive
early-exit curve is recomputed from the ENTROPY-head retrain (results_3p3/
entropy_head2, val-LOO-selected checkpoint). Baselines (crossattn/multimodn/shaspec/
decalign) and DyMo-proto are UNCHANGED. 4 datasets: iemocap | daliahar | pamap2 | dsads."""
import glob, json, os
import numpy as np
import matplotlib
matplotlib.use('Agg')
...
# seqA exit: ENTROPY-head retrain (val-LOO order, val-LOO-selected ckpt)
SEQ = json.load(open(os.path.join(R, 'seqA_gap_dual_exit_4ds.json')))
...
def seqa_exit(ds, which):
    M = M_OF[ds]; o = SEQ[ds]; ec = np.array(o['exit_curve']); gpk = o['gflops_per_k']
...
for col, ds in enumerate(DS):
    for row, (which, rk, ylab) in enumerate([('acc', 'best_test_acc', 'Accuracy'),
        ...
        ax.plot(gf, y, '-o', color='tab:blue', lw=2.4, ms=4, zorder=7,
                label='V6-seqA (ours, GAP-head dual, adaptive early-exit)')
        ...
        ax.set_xscale('log')   # per-panel x autoscale (no forced alignment)
...
fig.suptitle('GFLOPs vs Accuracy / Macro-F1 (cross-subject) — V6-seqA GAP-head dual adaptive early-exit (curve, val-LOO order) vs crossattn/multimodn/shaspec/decalign (full-modality) + DyMo-proto (greedy). mean/folds.\n'
             'seqA from seqA_gap_dual (val-LOO-selected ckpt); baselines + DyMo-proto unchanged. Per-panel x-axis (NOT aligned). IEMOCAP T=128 | daliahar T=256 | PAMAP2 T=512 sax_noisy | DSADS T=125.', fontsize=11)
fig.subplots_adjust(left=0.045, right=0.997, bottom=0.08, top=0.89, wspace=0.22, hspace=0.30)
o = os.path.join(R, 'gflops_acc_f1_seqA_gapdual_vs_baselines_perx.png')
fig.savefig(o, dpi=140); print('saved ->', o)
for ds in DS:
    gf, ya = seqa_exit(ds, 'acc')
    print(f"{ds:9s}({TAG[ds]}): seqA-gapdual-exit acc {ya.min():.3f}-{ya.max():.3f} @ GFLOPs {gf.min():.3f}-{gf.max():.3f} | "
