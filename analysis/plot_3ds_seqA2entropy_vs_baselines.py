...
SEQ = json.load(open(os.path.join(R, 'entropy_head2', 'exit_entropy2_4ds.json')))
...
for col, ds in enumerate(DS):
    for row, (which, rk, ylab) in enumerate([('acc', 'best_test_acc', 'Accuracy'),
        ...
        ax.plot(gf, y, '-o', color='tab:blue', lw=2.4, ms=4, zorder=7,
                label='V6-seqA (ours, entropy-head, adaptive early-exit)')
...
fig.suptitle('GFLOPs vs Accuracy / Macro-F1 (cross-subject) — V6-seqA ENTROPY-head adaptive early-exit (curve, val-LOO order) vs crossattn/multimodn/shaspec/decalign (full-modality) + DyMo-proto (greedy). mean/folds.\n'
             'seqA recomputed from entropy_head2 (val-LOO-selected ckpt); baselines + DyMo-proto unchanged. IEMOCAP T=128 | daliahar T=256 | PAMAP2 T=512 sax_noisy | DSADS T=125.', fontsize=11)
fig.subplots_adjust(left=0.045, right=0.997, bottom=0.08, top=0.89, wspace=0.22, hspace=0.30)
o = os.path.join(R, 'gflops_acc_f1_seqA2entropy_vs_baselines_3ds.png')
fig.savefig(o, dpi=140); print('saved ->', o)
for ds in DS:
    gf, ya = seqa_exit(ds, 'acc')
    print(f"{ds:9s}({TAG[ds]}): seqA-entropy-exit acc {ya.min():.3f}-{ya.max():.3f} @ GFLOPs {gf.min():.3f}-{gf.max():.3f} | "
