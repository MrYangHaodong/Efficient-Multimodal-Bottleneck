#!/usr/bin/env python
"""GFLOPs vs Acc / Macro-F1: V6-seqA adaptive early-exit curve vs DynMM / AdaMML
(routed points), on 5 datasets (4 HAR/affect cross-subject + CMU-MOSEI random
split). Extends plot_seqA_vs_dynmm_adamml.py with a MOSEI 5th column."""
import glob, json, os
import numpy as np
import matplotlib
matplotlib.use('Agg')
...
fig.suptitle('Modality-axis efficiency head-to-head: V6-seqA adaptive early-exit (curve) vs DynMM / AdaMML '
             '(routed operating point). DynMM/AdaMML use the SAME seqA experts (val-LOO subsets) / per-mod transformer + policy.\n'
             'IEMOCAP/daliahar/pamap2/dsads (cross-subject) | CMU-MOSEI T=50 Acc-2 (random split, GloVe). '
             'On MOSEI text dominates -> DynMM routes 100%% to the text expert; AdaMML selects text-only.', fontsize=10.5)
fig.subplots_adjust(left=0.037, right=0.997, bottom=0.08, top=0.88, wspace=0.27, hspace=0.30)
o = os.path.join(R, 'gflops_seqA_vs_dynmm_adamml_5ds.png')
...
for ds in DS:
    ...
    print(f"{ds:9s}: seqA {ya.min():.3f}-{ya.max():.3f}@{gf.min():.2f}-{gf.max():.2f}GF | "
          f"DynMM {ad:.3f}@{gd:.3f}(seqA@same {se_d:.3f}) | AdaMML {aa:.3f}@{ga:.3f}(seqA@same {se_a:.3f})")
