...
NAME = {'crossattn': 'MAESTRO', 'multimodn': 'MultiModN', 'shaspec': 'ShaSpec', 'decalign': 'DeCAlign'}
...
for col, ds in enumerate(DS):
    for row, (which, rk, ylab) in enumerate([('acc', 'best_test_acc', 'Accuracy'),
        ...
        ax.plot(gf, y, '-o', color='tab:blue', lw=2.4, ms=4, zorder=7,
                label='Ours (sequential + early-exit)')
        for method, (color, mk) in STYLE.items():
            g = base_gflops(method, ds); a = base_acc(method, ds, rk)
            if g is not None and a is not None:
                ax.plot([g], [a], mk, color=color, ms=12, zorder=6, label=NAME.get(method, method))
...
fig.subplots_adjust(left=0.045, right=0.997, bottom=0.08, top=0.96, wspace=0.22, hspace=0.30)
o = os.path.join(R, 'gflops_acc_f1_seqA_vs_baselines_perx.png')
