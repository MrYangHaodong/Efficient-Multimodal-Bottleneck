"""ShaSpec baseline training on IEMOCAP with the V6 **dyna**
(grad-norm-based asymmetric modality-dropout) curriculum.

IEMOCAP-specific notes:
  * 6 modalities with heterogeneous channel widths.
  * Raw float features (``input_mode='float'``) — no SAX tokenisation.
  * 3 stratified 70/15/15 splits (``--fold = 0/1/2``).
  * 4-class emotion classification.

Schedule mirrors ``main_v6_iemocap_fusion_only_dyna.py``.
"""
...
from data.IEMOCAP.get_data import (                                        # noqa: E402
...
class ShaSpecIEMOCAPDynaWrapper(nn.Module):
    """Unpacks 2M+1 IEMOCAP batch -> ``[B, T, sum(variates)]`` concat.
    :class:`GenericShaSpecClassifier` applies per-modality dropout via
...
parser = argparse.ArgumentParser(
    description='ShaSpec on IEMOCAP with V6 grad-norm dyna modality-drop.')

parser.add_argument('--exp_name', default='shaspec_iemocap_dyna', type=str)
parser.add_argument('--results_dir',
                    default='./results_shaspec_iemocap_dyna/', type=str)
...
model = ShaSpecIEMOCAPDynaWrapper(inner, modalities)
...
config = {
    ...
    'model_variant': 'shaspec_iemocap_dyna',
