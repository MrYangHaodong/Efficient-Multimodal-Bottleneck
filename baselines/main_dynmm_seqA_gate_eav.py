...
Mirrors main_dynmm_seqA_gate.py (daliahar/wesad/pamap2) but adapted to EAV's
...
splits via data.EAV.get_data.get_dataloader(split_mode='cross_subject').

3 frozen seqA experts on nested modality subsets (1mod / partial / full, see
dynmm_branches.json["eav"]) act as branches. A lightweight per-sample gate
...
Run from clean_models/train/:
  python main_dynmm_seqA_gate_eav.py --dataset eav --fold 0 \
      --cuda_pick cuda:0 --results_dir ../results_3p3/dynmm_eav/dynmm_seqA \
...
# EAV plumbing (mirrors compute_seqa_loo_eav.py)
DATA_ROOT = '/files1/haodong/data/EAV'
SPLIT_MODE = 'cross_subject'   # cross-subject session-disjoint splits (per fold)
RESROOT = '../results_3p3/dynmm_eav'   # where the 3 experts live
ALL_MODS = list(EAVDataset.ALL_MODALITIES)   # ['video','audio','text','mocap_hand','mocap_head','mocap_rotated']
NUMC = NUM_CLASSES
BR_MODS = json.load(open(os.path.join(os.path.dirname(os.path.abspath(__file__)),
                                      'dynmm_branches.json')))[args.dataset]


# ---- expert build (mirrors compute_seqa_loo_eav.build, restricted to subset) ----
class _EAVV6Cfg:
    """Replicates the cfg shim from the EAV seqA training script, restricted
    to a branch's modality SUBSET."""
    def __init__(self, modalities):
        self.modalities = list(modalities)
        self.variates = {m: FEATURE_DIMS[m] for m in self.modalities}
        # Match the training _EAVV6Cfg EXACTLY: the dual-video bottleneck path is
        # always sized to FEATURE_DIMS['video'] (1024) regardless of the subset,
        # so the frozen expert checkpoints load without an input_projectors.video
        # shape mismatch (audio-only expert still has a 1024-d video projector).
        v = FEATURE_DIMS.get('video', 1024)
        self.video_high_dim = v
        self.video_low_dim = v
        self.num_classes = NUM_CLASSES


def build_expert(C, modalities, input_length):
    cfg = _EAVV6Cfg(modalities)
...
def unpack(batch):
    """EAV collate -> (high0, low0, high1, low1, ..., label[B,1]) over ALL_MODS
...
# EAV modalities have DIFFERENT feature dims, so we cannot concat a single
...
def _seqa_loader_cfg():
    seqa = (sorted(glob.glob('./results_eav/seqA*/*')) +
            sorted(glob.glob(os.path.join(RESROOT, 'dynmm_experts', '*', '*'))) +
            sorted(glob.glob('../results_3p3/seqA_eav/*')))
    for d in seqa:
...
_CKPT_NAMES = ['best_model_loo_fold{f}.pth', 'best_model_fold{f}.pth', 'best_model_rand_fold{f}.pth']


def _expert_ckpt(rundir, fold):
    for nm in _CKPT_NAMES:
        p = os.path.join(rundir, nm.format(f=fold))
        if os.path.isfile(p):
            return p
    return None
...
def _resolve_expert(br, fold):
    ...
    rd = [d for d in rd if _expert_ckpt(d, fold) is not None
...
for br in BRANCHES:
    ...
    sd = torch.load(_expert_ckpt(rundir, args.fold),
