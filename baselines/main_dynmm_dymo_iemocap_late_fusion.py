"""DynMM / DyMo on IEMOCAP, V6 backbone (2-layer per-modality encoder +
4-layer bottleneck fusion) reused as the modality-subset encoder/fusion.

IEMOCAP sibling of `main_dynmm_dymo_daliahar_late_fusion.py`.
...
Run:
    python main_dynmm_dymo_iemocap_late_fusion.py --method dynmm --fold 0 --cuda_pick cuda:0
    python main_dynmm_dymo_iemocap_late_fusion.py --method dymo  --fold 0 --cuda_pick cuda:0
    python main_dynmm_dymo_iemocap_late_fusion.py --method dynmm --aggregate
"""
...
from multimodal_model.crossmodal_transformer import (CrossModalTransformer, CrossModalDyMo, CrossModalDynMM, IEMOCAPDyMoWrapper, IEMOCAPDynMMWrapper)
...
parser.add_argument('--exp_name', default='dynmm_dymo_late_fusion_cs', type=str)
...
parser.add_argument('--results_dir', default='./results_dynmm_dymo_iemocap/', type=str)
...
print(f"IEMOCAP split id: {fold_id} (fold={args.fold})  split_mode={args.split_mode}")
...
# cross-modal fusion. IEMOCAP feeds a feats-list; the IEMOCAP wrappers concat
# the per-modality highs to [B,T,sum(V)] before the cross-modal transformer.
...
if args.method == 'dynmm':
    ...
    model = IEMOCAPDynMMWrapper(
        CrossModalDynMM(inner, branches=branches, gate_hidden=args.gate_hidden), modalities)
    print(f'DynMM branches: {model.branches}  costs={model.costs.tolist()}')
else:
    model = IEMOCAPDyMoWrapper(CrossModalDyMo(inner), modalities)
...
def unpack(batch, device):
    *feats, label = batch
    ...
    return feats, label.squeeze(-1).long().to(device)
...
config = {
    ...
    'loss': args.loss, 'data_root': args.data_root,
