"""DMBF-style baseline on CMU-MOSEI (Acc-2), comparable to our other MOSEI baselines.

Per-modality MSP-confidence gating + spatial-temporal attention fusion + CRL
(correctness-ranking loss). Same data pipeline / metrics / results-json schema as
``main_shaspec_mosei_dyna.py`` so it drops into the MOSEI comparison.

  python main_dmbf_mosei.py --fold 0 --seed_num 42 --cuda_pick cuda:0
  python main_dmbf_mosei.py --aggregate            # mean/std over results_fold{0,1,2}.json
"""
...
from data.CMU_MOSEI.get_data import (                                        # noqa: E402
    get_dataloader as mosei_get_dataloader, collate_fn as mosei_collate,
    FEATURE_DIMS, CMUMOSEIDataset, get_num_classes)
from multimodal_model.dmbf_baseline import (                                 # noqa: E402
    DMBFClassifier, CorrectnessHistory, dmbf_loss)

ap = argparse.ArgumentParser(description='DMBF-style on CMU-MOSEI')
ap.add_argument('--exp_name', default='dmbf_mosei')
ap.add_argument('--results_dir', default='./results_dmbf_mosei/')
...
def _idx_collate(batch):
    ...
    out = mosei_collate([b[:-1] for b in batch])          # (padded..., label(B,1))
...
res = {'experiment_name': exp_name_full, 'model_variant': 'dmbf_mosei', 'fold': args.fold,
