...
Mirrors main_dynmm_seqA_gate.py (daliahar/wesad/pamap2) but adapted to IEMOCAP's
...
splits via data.IEMOCAP.get_data.get_dataloader(split_mode='cross_subject').

3 frozen seqA experts on nested modality subsets (1mod / partial / full, see
dynmm_branches.json["iemocap"]) act as branches. A lightweight per-sample gate
...
Run from clean_models/train/:
  python main_dynmm_seqA_gate_iemocap.py --dataset iemocap --fold 0 \
      --cuda_pick cuda:0 --results_dir ../results_3p3/dynmm_iemocap/dynmm_seqA \
...
ap.add_argument('--dataset', default='iemocap', choices=['iemocap'])
...
# IEMOCAP plumbing (mirrors compute_seqa_loo_iemocap.py)
DATA_ROOT = '/files1/haodong/data/IEMOCAP'
SPLIT_MODE = 'cross_subject'   # cross-subject session-disjoint splits (per fold)
RESROOT = '../results_3p3/dynmm_iemocap'   # where the 3 experts live
ALL_MODS = list(IEMOCAPDataset.ALL_MODALITIES)   # ['video','audio','text','mocap_hand','mocap_head','mocap_rotated']
NUMC = NUM_CLASSES
BR_MODS = json.load(open(os.path.join(os.path.dirname(os.path.abspath(__file__)),
                                      'dynmm_branches.json')))[args.dataset]


# ---- expert build (mirrors compute_seqa_loo_iemocap.build, restricted to subset) ----
class _IEMOCAPV6Cfg:
    """Replicates the cfg shim from the IEMOCAP seqA training script, restricted
...
def unpack(batch):
    """IEMOCAP collate -> (high0, low0, high1, low1, ..., label[B,1]) over ALL_MODS
...
# IEMOCAP modalities have DIFFERENT feature dims, so we cannot concat a single
...
def _seqa_loader_cfg():
    seqa = sorted(glob.glob('../results_3p3/seqA_iemocap/*'))
    for d in seqa:
...
def _resolve_expert(br, fold):
    ...
    rd = [d for d in rd if os.path.isfile(os.path.join(d, f'best_model_fold{fold}.pth'))
...
if missing:
    print('[experts not ready] No trained seqA expert checkpoint found for '
          f'iemocap fold{args.fold} branches: {missing}.')
    ...
    # Still verify the IEMOCAP pipeline + gate build before exiting.
    Lc = _seqa_loader_cfg()
    if Lc is None:
        print('[experts not ready] (could not read a seqA config for loader params; '
              'skipping dataset/gate build verification).')
        sys.exit(0)
    print('[verify] building IEMOCAP cross-subject dataset + gate (experts skipped)...')
    train_loader, val_loader, test_loader = iemocap_get_dataloader(
...
Lc = _seqa_loader_cfg()
assert Lc is not None, 'could not read a seqA_iemocap config for loader params'
...
for br in BRANCHES:
    ...
    sd = torch.load(os.path.join(rundir, f'best_model_fold{args.fold}.pth'),
