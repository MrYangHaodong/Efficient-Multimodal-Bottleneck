"""MultiModN baseline training on IEMOCAP with the V6 **dyna**
...
IEMOCAP specifics mirror ``main_flexmoe_iemocap_dyna.py``:
...
Run:
    python main_multimodn_iemocap_dyna.py --fold=0 --cuda_pick=cuda:0 \
...
class MultiModNIEMOCAPDynaWrapper(nn.Module):
    """Unpacks the IEMOCAPDataset 2M+1 batch and concatenates the high
...
parser = argparse.ArgumentParser(
    description='MultiModN on IEMOCAP with V6 grad-norm dyna modality-drop.')

parser.add_argument('--exp_name', default='multimodn_iemocap_dyna', type=str)
parser.add_argument('--results_dir',
                    default='./results_multimodn_iemocap_dyna/', type=str)
...
# MultiModN on IEMOCAP -- raw float features (no SAX). Structure aligned to
...
model = MultiModNIEMOCAPDynaWrapper(inner, modalities)
...
config = {
    ...
    'model_variant': 'multimodn_iemocap_dyna',
