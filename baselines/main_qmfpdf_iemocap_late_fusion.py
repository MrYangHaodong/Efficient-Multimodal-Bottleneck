"""QMF / PDF confidence-weighted **late fusion** on IEMOCAP.

IEMOCAP sibling of `main_qmfpdf_daliahar_late_fusion.py`.  Same V6 backbone
...
Differences vs DaliaHAR:
  - up to 6 heterogeneous modalities; `IEMOCAPDataset` returns a list of
...
Run:
    python main_qmfpdf_iemocap_late_fusion.py --fusion_method qmf --fold 0 --cuda_pick cuda:0
    python main_qmfpdf_iemocap_late_fusion.py --fusion_method pdf --fold 0 --cuda_pick cuda:0
    python main_qmfpdf_iemocap_late_fusion.py --fusion_method qmf --aggregate
"""
...
class QMFPDFIEMOCAPWrapper(nn.Module):
    """Unpack the IEMOCAPDataset batch (2*M modality tensors) into V6's two
...
parser = argparse.ArgumentParser(
    description='QMF / PDF confidence-weighted late fusion on IEMOCAP.')
...
parser.add_argument('--results_dir', default='./results_qmfpdf_iemocap/', type=str)
...
# NO cross-modal fusion network. IEMOCAP feeds a feats-list, so the wrapper
