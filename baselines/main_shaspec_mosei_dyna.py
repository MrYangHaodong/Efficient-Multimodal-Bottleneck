"""ShaSpec baseline training on MOSEI with the V6 **dyna**
(grad-norm-based asymmetric modality-dropout) curriculum.

MOSEI-specific notes:
...
class ShaSpecMOSEIDynaWrapper(nn.Module):
    """Unpacks 2M+1 MOSEI batch -> ``[B, T, sum(variates)]`` concat.
...
parser = argparse.ArgumentParser(
    description='ShaSpec on MOSEI with V6 grad-norm dyna modality-drop.')

parser.add_argument('--exp_name', default='shaspec_mosei_dyna', type=str)
parser.add_argument('--results_dir',
                    default='./results_shaspec_mosei_dyna/', type=str)
...
model = ShaSpecMOSEIDynaWrapper(inner, modalities)
...
config = {
    ...
    'model_variant': 'shaspec_mosei_dyna',
