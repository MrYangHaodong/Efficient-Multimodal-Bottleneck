"""DynMM / DyMo on EAV, V6 backbone (2-layer per-modality encoder +
4-layer bottleneck fusion) reused as the modality-subset encoder/fusion.

EAV sibling of `main_dynmm_dymo_daliahar_late_fusion.py`.
...
Run:
    python main_dynmm_dymo_eav_late_fusion.py --method dynmm --fold 0 --cuda_pick cuda:0
    python main_dynmm_dymo_eav_late_fusion.py --method dymo  --fold 0 --cuda_pick cuda:0
    python main_dynmm_dymo_eav_late_fusion.py --method dynmm --aggregate
"""
...
parser = argparse.ArgumentParser(
    description='DynMM / DyMo on EAV (V6 2-enc + 4-fusion backbone).')
...
parser.add_argument('--results_dir', default='./results_dynmm_dymo_eav/', type=str)
...
parser.add_argument('--data_root', default='/files1/haodong/data/EAV', type=str)
...
v6_cfg = _EAVV6Cfg(modalities)
# Cross-modal Transformer (NO V6): 2-layer per-modality encoder + 4-layer
# cross-modal fusion. EAV feeds a feats-list; the EAV wrappers concat
