#!/usr/bin/env bash
# Train V6 fusion-only (uniform modality dropout) on IEMOCAP, 3 splits → aggregate.
# Usage:
#   bash run_iemocap_fusion_only.sh            # train all 3 splits in parallel
#   bash run_iemocap_fusion_only.sh --aggregate  # re-run aggregate only

set -euo pipefail
cd "$(dirname "$0")"

# ── configurable ──────────────────────────────────────────────────────────────
EXP_NAME="iemocap_fusion_only"
...
SCRIPT="main_v6_iemocap_fusion_only.py"
CONDA_ENV="maestro"
...
run_split() {
    ...
    conda run -n "${CONDA_ENV}" python "${SCRIPT}" \
        ...
        --mod_drop_warmup="${MOD_DROP_WARMUP}" \
        --mod_drop_ramp_epochs="${MOD_DROP_RAMP}" \
