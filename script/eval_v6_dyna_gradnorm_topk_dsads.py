"""DSADS sibling of `eval_v6_dyna_gradnorm_topk.py`.

Top-K modality eval driven by per-modality gradient norms collected
during `main_v6_dsads_fusion_only_dyna.py` training.  Loads DSADS via
scenario .pt files (no subject folds).
"""
...
parser.add_argument('--val_from_trg', default=False, type=_str2bool,
                    help='If true, eval only on the 70%% portion of trg '
                         '(matches the selector training where 30%% trg is '
                         'val).  Ensures apples-to-apples test_acc.')
parser.add_argument('--val_ratio_trg', default=0.3, type=float)
...
M = len(modalities)
print(f"\nDSADS scenario index: {args.fold}")

trg_path = os.path.join(args.data_root,
                        f'dsad_diversify_dict_scenario{args.fold}_trg.pt')
test_data_full = torch.load(trg_path, weights_only=False)
if args.val_from_trg:
    from sklearn.model_selection import train_test_split as _tts
    _, te_x, _, te_y = _tts(
        test_data_full['samples'], test_data_full['labels'],
        test_size=1.0 - args.val_ratio_trg,
        random_state=args.seed_num, stratify=test_data_full['labels'])
    test_data = {'samples': te_x, 'labels': te_y}
    print(f"Eval on 70% trg = {len(te_x)} samples (val_from_trg=True)")
else:
