...
inputs** + dyna gradnorm-asymmetric modality dropout.

Compared to ``main_v6_iemocap_fusion_only_dyna.py``:

  1. Backbone = **orig** V6 fusion
     (``multimodal_model.v6_downsample_orig.DualVideoBottleneckModelV6Downsample``)
     which uses a ``ModuleDict`` of independent per-modality encoders
     rather than the bmm-batched stacked encoder.  This natively
     supports each modality having its own sequence length per batch.

  2. ``IEMOCAPDataset`` is invoked with ``use_batched_fusion=False``,
     so the loader **keeps raw temporal lengths for short modalities**
     (T < max_seq_len → no zero-padding); only modalities with
     T > max_seq_len get interpolated down to max_seq_len.  Within
     each batch ``pad_sequence`` pads each modality to its own
     batch-max length — NOT a global max across modalities.

  3. ``sparse_attn_variant='opt'`` is enforced (the bmm-stratified
     variants live in the *_opt_batched* file and aren't available
     in orig anyway).

Dropout curriculum + gradient-norm asymmetric scaling are kept
identical to the dyna script so we can isolate the effect of
"let each modality keep its native length".

Run:
    python main_v6_iemocap_fusion_only_varlen.py --fold=0 --cuda_pick=cuda:0
    python main_v6_iemocap_fusion_only_varlen.py --aggregate
"""
...
parser.add_argument('--exp_name', default='v6_fusion_only_varlen_dyna', type=str)
...
config = {
    ...
    'model_variant': 'v6_fusion_only_varlen_dyna',
