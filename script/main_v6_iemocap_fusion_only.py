"""V6 fusion-only training on IEMOCAP (NO modality selector).

Stripped-down sibling of `main_v6_daliahar_fusion_only.py`.  This
script trains the V6Downsample backbone with `no_selector=True`, so the
network is just:

    per-modal embedding → BatchedModalityEncoder
...
Run:
    python main_v6_iemocap_fusion_only.py --split_id=0 --cuda_pick=cuda:0
    python main_v6_iemocap_fusion_only.py --split_id=1 --cuda_pick=cuda:1
    python main_v6_iemocap_fusion_only.py --split_id=2 --cuda_pick=cuda:2
    python main_v6_iemocap_fusion_only.py --aggregate
"""
...
class V6FusionOnlyWrapper(nn.Module):
    """Adapts the V6 model's dict-input interface to IEMOCAP's per-modality
    tensor batches produced by ``get_dataloader`` + ``prepare_batch``.
    ...
    def forward(self, high, low, modality_drop_prob=0.0, training=False):
        if training and modality_drop_prob > 0:
            ...
            # survives.  p^M ≈ 1% at p=0.4, M=6 — almost never iterates twice.
            for _ in range(8):
...
def train_one_epoch(loader, model, criterion, optimizer, epoch, device,
    ...
    for i, batch in enumerate(loader):
        ...
        logits = model(high, low, modality_drop_prob=modality_drop_prob,
                       training=True)
...
def evaluate_one_epoch(loader, model, criterion, epoch, device):
    ...
    for i, batch in enumerate(loader):
        high, low, labels = prepare_batch(batch, device)
        logits = model(high, low, modality_drop_prob=0.0, training=False)
        loss = criterion(logits, labels)
        _, predicted = torch.max(logits, 1)
        acc = predicted.eq(labels).sum().item() / labels.size(0)
        loss_meter.update(loss.item(), labels.size(0))
...
parser = argparse.ArgumentParser(
    description='V6 fusion-only training on IEMOCAP (no selector).')
...
model = V6FusionOnlyWrapper(inner, modalities)
