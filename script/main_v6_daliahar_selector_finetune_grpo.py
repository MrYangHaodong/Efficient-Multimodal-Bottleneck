"""V6 stage-2 selector training on DaliaHAR with **GRPO** policy gradient
(replaces the broken vanilla REINFORCE-from-EMA-baseline path).

Same setup as the non-GRPO sibling but wraps the V6Downsample model with
``V6GRPOWrapper``: each forward draws G K-subset action samples via
Gumbel-top-K, computes per-action rewards, and uses group-relative
standardised advantages for the policy gradient.

Why this variant exists:
  Vanilla REINFORCE in the IMS path was empirically 0 (advantage was
  zero-mean per state and log_p_action was nearly constant across the
  batch because top-K is shared batch-level).  GRPO injects per-state
  action diversity by drawing G different K-subsets for the SAME state,
  giving non-zero policy-gradient signal.  See the wrapper file for
  details.

Lambdas:
  - lambda_grpo  scales the policy-gradient term
  - lambda_kl    KL anchor to slow-moving EMA reference policy
  - lambda_entropy  encourages exploration (drives sigmoid away from 0/1)
  - lambda_probe / lambda_diversity stay the same as the non-GRPO script

Run:
    python main_v6_daliahar_selector_finetune_grpo.py \\
        --fold=0 --cuda_pick=cuda:0 \\
        --frozen_ckpt=./results_v6_fusion_dyna/<exp>/best_model_fold0.pth
"""
...
from selector.v6_grpo_wrapper import V6GRPOWrapper
...
def train_one_epoch(loader, model, criterion, optimizer, epoch, device,
    ...
    for i, (x, y) in enumerate(loader):
        ...
        logits, _, aux_losses = model(x, modality_drop_prob=modality_drop_prob,
...
parser.add_argument('--exp_name', default='v6_selector_finetune_grpo', type=str)

# ---------------- GRPO knobs ----------------
parser.add_argument('--n_grpo_samples', default=4, type=int,
                    help='G — number of action samples per batch.')
parser.add_argument('--lambda_grpo', default=0.1, type=float)
parser.add_argument('--lambda_kl', default=0.02, type=float,
                    help='KL anchor weight to slow-moving reference policy.')
parser.add_argument('--lambda_entropy', default=0.01, type=float,
                    help='Entropy bonus weight (drives sigmoid out of saturation).')
parser.add_argument('--clip_eps', default=0.2, type=float,
                    help='Optional clip on (log_p_new - log_p_old) for stability.')
parser.add_argument('--grpo_eps', default=1e-8, type=float)
parser.add_argument('--gumbel_temperature', default=20.0, type=float,
                    help='Flatten log-scores before Gumbel-top-K so the G '
                         'group members actually differ (sigmoid IMS is '
                         'often very confident → low-τ Gumbel locks all g '
                         'to the same K-subset).')
...
inner = DualVideoBottleneckModelV6Downsample(
    ...
    # GRPO replaces vanilla REINFORCE — force inner's lambda_reinforce to 0
    # so the broken zero-signal RL loss isn't added.
...
# GRPO wrapper — handles G-sample action draws + group-relative advantages.
# State dict layout: keys start with `model.<inner_attr>` so the existing
# fusion ckpt loads cleanly with strict=False.
model = V6GRPOWrapper(
    inner, modalities, dict(dataset_cfg.variates),
    n_grpo_samples=args.n_grpo_samples,
