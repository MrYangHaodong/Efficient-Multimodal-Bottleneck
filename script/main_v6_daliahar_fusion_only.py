"""V6 fusion-only training on DaliaHAR (NO modality selector).

Stripped-down sibling of `main_robust_v6_daliahar_unifmod.py`.  This
script trains the V6Downsample backbone with `no_selector=True`, so the
network is just:

    per-modal SAX/embedding → BatchedModalityEncoder
                            → SimpleMBTFusionAdaptiveMLPDownsampleBmm
                            → global pool → classifier head

No probe heads, no IMS, no probe / diversity / reinforce / sparsity
auxiliary losses, no top-K schedule, no PDF swap.

Modality-dropout schedule (uniform across modalities) is kept — it's
training data augmentation, not part of the selector machinery.

Run:
    python main_v6_daliahar_fusion_only.py --fold=0 --cuda_pick=cuda:0
    python main_v6_daliahar_fusion_only.py --fold=1 --cuda_pick=cuda:1
    python main_v6_daliahar_fusion_only.py --fold=2 --cuda_pick=cuda:2
    python main_v6_daliahar_fusion_only.py --aggregate
"""
...
# Make `utils`, `data_utils`, `models` from this export tree importable.
...
class V6FusionOnlyWrapper(nn.Module):
    """Adapts the V6 model's dict-input interface to a concatenated
    `[B, T, C_total]` tensor produced by `HARDataset`.

    At training time, modality dropout works by **completely removing**
    each modality from the V6 forward input dict with independent
    Bernoulli probability `modality_drop_prob`.  Removed modalities
    don't enter the per-modal projection / encoder / fusion pipeline at
    all — the model only spends compute on the surviving subset, which
    simulates random K-selection without an explicit selector head.

    At least one modality is always kept per batch so the forward pass
    ...
    def forward(self, x, modality_drop_prob=0.0, training=False):
        ...
        if training and modality_drop_prob > 0:
            # Independent Bernoulli per modality: with prob `p`, REMOVE
            # this modality from the V6 input dict entirely (it never
            # reaches projection / encoder / fusion).  Same subset is
            # used for the whole batch to keep the batched encoder happy.
            #
            # Rejection sampling: re-roll until at least one modality
            # survives, so the per-modality marginal stays Bernoulli(1-p)
            # conditioned on K >= 1.  Probability of empty draw is p^M
            # (≈1% at p=0.4, M=5) so this loop almost never iterates twice.
            for _ in range(8):
                ...
                # Numerical edge case (e.g., p≈1): force keep one modality
                # uniformly at random.  Bias on marginal is negligible
                # because this branch is virtually unreachable for sane p.
...
parser = argparse.ArgumentParser(
    description='V6 fusion-only training on DaliaHAR (no selector).')

parser.add_argument('--exp_name', default='v6_fusion_only', type=str)
parser.add_argument('--results_dir', default='./results_v6_fusion/', type=str)
...
for epoch in range(num_epochs):
    ...
    # Warm best: only after modality-drop plateau
