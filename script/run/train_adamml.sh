#!/usr/bin/env bash
# AdaMML policy-gated adaptive baseline — d_model=128.
#
# Three-phase schedule (10 warmup + 20 joint + 10 finetune = 40 epochs)
# instead of a single --num_epochs. Dropout curriculum: warmup 10 / ramp 20.
#
# Usage:
#   bash script/run/train_adamml.sh <dataset> [GPU_ID]
#   bash script/run/train_adamml.sh iemocap 0
#
...
DATASET="${1:?usage: bash script/run/train_adamml.sh <dataset> [GPU_ID]}"
GPU="${2:-0}"
...
for FOLD in $FOLDS; do
  echo "==== adamml $DATASET fold$FOLD -> GPU $GPU ===="
  "$PYTHON" "$REPO/script/training/adamml.py" \
    --dataset "$DATASET" --fold "$FOLD" \
    "${DATA_ARGS[@]}" \
    --d_model 128 --num_layers 6 \
    --warmup_epochs 10 --joint_epochs 20 --finetune_epochs 10 \
    --drop_warmup 10 --drop_ramp 20 --max_drop_prob 0.4 \
    --lr 1e-4 --batch_size "$BS" --num_workers 4 --cuda_pick cuda:0 \
    --results_dir "$RESULTS" --exp_name adamml_$DATASET
done

echo "==== DONE: adamml $DATASET all folds ===="
