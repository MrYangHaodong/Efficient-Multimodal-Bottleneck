...
Wraps the genuine shared-bottleneck-token fusion `SimpleMBTFusion`
(multimodal_model/bottleneck_fusion.py) with a per-modality input split (from the
concatenated (B, T, sum_variates) tensor the ee_data harness produces) and a linear
classifier head. NO early-exit / gating / threshold / modality selection — a single
forward pass always using ALL modalities. This is the non-adaptive counterpart of the
seqA bottleneck fusion, at the same structure params (d_model / num_layers / nhead /
fusion_layer / n_bottlenecks).
"""
...
# SimpleMBTFusion is vendored into this package (from the MAESTRO repo), so no
# sibling-repo sys.path hack is needed and there is no `models` name collision.
from multimodal_model.bottleneck_fusion import SimpleMBTFusion
