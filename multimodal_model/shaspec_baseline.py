"""ShaSpec baseline classifier (generic, multi-dataset) for the
v6_organized dyna training scripts.

Generalised from the inline ``DaliaShaSpecClassifier`` defined in
``main_shaspec_daliahar_unifmod.py`` at the old MAESTRO root.  Supports
both raw multivariate (``input_mode='float'``) and SAX-token inputs
(``input_mode='sax'``).  ``modality_dropout_probs`` (scalar or per-modality
dict) is applied directly inside ``forward`` so the script-level wrapper
can stay trivial.
"""
...
def _build_channel_map(modalities, variates):
    ...
    for mod in modalities:
        width = variates[mod]
        mapping[mod] = list(range(start, start + width))
        start += width
    return mapping
