"""MultiModN baseline classifier (generic, multi-dataset) for the
...
class GenericMultiModNClassifier(nn.Module):
    ...
    def forward(self, x, modality_dropout_probs=None, training=False, keep=None):
        ...
        for i in order:                                           # i = modality index
            ...
            keep = observed[:, i].to(cand.dtype).unsqueeze(-1)    # (B,1)
            state = keep * cand + (1.0 - keep) * state            # skip if dropped
