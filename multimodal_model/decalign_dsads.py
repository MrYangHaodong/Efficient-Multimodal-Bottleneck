"""
DecAlign — DSADS adaptation (faithful port from /files1/haodong/DecAlign).

Generalizes the original 3-modality (text/audio/video) DecAlign architecture
to M-modality multivariate time series:
  ...
  • Three-path fusion:
      A. Transformer-fusion on concat([s_m for m in modalities], dim=feature)
      B. Cross-modal attention: M·(M-1) directed cross-attn + M self-memory
         transformers; each modality m absorbs M-1 others, then self-mixes.
      C. Homogeneity branch: avg-pool the common features, concat.
    Final = cat(hete = A + B, homo = C) → Linear classifier.

Total loss: task_loss + alpha1·decouple_loss + alpha2·(hete_loss + homo_loss).

Compute warning:
  • Multi-marginal Sinkhorn for M=5 needs a K^5 tensor (K=5 → 3125 entries,
    K=10 → 100K). For DSADS M=5, K=5 is recommended.
  • M·(M-1) cross-attention modules for M=5 = 20 (vs 6 in DecAlign-original).
    Plus M self-mem transformers. Param count ~3x DecAlign-original.

Usage:
    from models.decalign_dsads import DecAlignDSADS
    model = DecAlignDSADS(cfg=DSADS(), num_classes=19, input_length=125,
                          d_model=64, nhead=8, num_layers=3,
                          num_prototypes=5, ot_reg=0.1, ot_num_iters=50,
                          alpha1=0.1, alpha2=0.01)

    # Forward expects {modality: [B, T, V_m]} (or pre-concatenated [B, T, sum(V)])
    out = model(x_dict)   # → dict with 'logits', 'dec_loss', 'hete_loss', 'homo_loss'
    loss = ce(out['logits'], y) + model.alpha1 * out['dec_loss'] \
           + model.alpha2 * (out['hete_loss'] + out['homo_loss'])
"""
...
# ---------------------------------------------------------------------------
# DecAlign — DSADS / generic M-modality
# ---------------------------------------------------------------------------
class DecAlignDSADS(nn.Module):
    ...
    def forward(self, x):
        ...
        Returns:
            ...
            and the intermediate tensors s_*, c_*, fusion_rep_*.
        """
        ...
        # a token_dim float vector, flatten the V_m × token_dim axes, then
