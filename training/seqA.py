...
def build_data(args, device):
    """Dispatch on ``args.dataset``; reproduce that dataset's loader + (high,low,y)
    conversion, build the model, and return preloaded train/val/test batches.

    Returns:
        (train_batches, val_batches, test_batches, mods, num_classes, model)
        where each *_batches is a list of (high_dict, low_dict, y) on CPU.
    """
    ds = args.dataset
    ...
    if ds == 'iemocap':
        ...
        mods = (list(args.modalities) if args.modalities
                else list(IEMOCAPDataset.ALL_MODALITIES))
        ...
        tl, vl, el = iemocap_get_dataloader(
            data_root=args.data_root or "/files1/haodong/data/IEMOCAP", batch_size=args.batch_size,
            num_workers=args.num_workers, modalities=mods, split_id=fold_id,
        ...
        model = _make_model(args, mods, variates, num_classes, input_length,
                            v_dim, v_dim, device)
        return (_preload_interleaved(tl, mods), _preload_interleaved(vl, mods),
                _preload_interleaved(el, mods), mods, num_classes, model)
    ...
    if ds == 'eav':
        ...
        mods = (list(args.modalities) if args.modalities
                else list(EAVDataset.ALL_MODALITIES))
        variates = {m: FEATURE_DIMS[m] for m in mods}
        num_classes = NUM_CLASSES
        v_dim = FEATURE_DIMS.get('video', 1024)
        fold_id = args.fold if args.fold is not None else 0
        tl, vl, el = eav_get_dataloader(
            data_root=args.data_root or "/files1/haodong/data/EAV", batch_size=args.batch_size,
            num_workers=args.num_workers, modalities=mods, split_id=fold_id,
            train_shuffle=True, max_seq_len=args.max_seq_len,
            time_compression_ratio=args.time_compression_ratio,
            use_batched_fusion=True, available_sessions=args.available_sessions,
    ...
    if ds == 'mosei':
        from data.CMU_MOSEI.get_data import (
            ...
            FEATURE_DIMS, CMUMOSEIDataset,
        )
        mods = (list(args.modalities) if args.modalities
                else list(CMUMOSEIDataset.ALL_MODALITIES))
        variates = {m: FEATURE_DIMS[m] for m in mods}
        num_classes = args.mosei_task   # Acc-2/5/7 -> 2/5/7 classes
        _task = {2: 'classification2', 5: 'classification5', 7: 'classification7'}[args.mosei_task]
        v_dim = FEATURE_DIMS.get('video', 1024)
        fold_id = args.fold if args.fold is not None else 0
        tl, vl, el = mosei_get_dataloader(
            data_root=args.data_root or "/files1/haodong/data/CMU-MOSI/CMU-MOSEI", batch_size=args.batch_size,
            num_workers=args.num_workers, modalities=mods, split_id=fold_id,
            train_shuffle=True, max_seq_len=args.max_seq_len,
            time_compression_ratio=args.time_compression_ratio,
            use_batched_fusion=True, available_sessions=args.available_sessions,
            split_mode=args.split_mode, task=_task,
        )
        input_length = args.max_seq_len if args.max_seq_len > 0 else 50
        model = _make_model(args, mods, variates, num_classes, input_length,
                            v_dim, v_dim, device)
        return (_preload_interleaved(tl, mods), _preload_interleaved(vl, mods),
                _preload_interleaved(el, mods), mods, num_classes, model)
    ...
    def _channel_map(mods, variates):
        sl, off = {}, 0
        for m in mods:
            sl[m] = (off, off + variates[m])
            off += variates[m]
        return sl
    ...
    if ds == 'daliahar':
        ...
        if args.modalities:
            ...
            dcfg.modalities = [m for m in dcfg.modalities if m in keep]
            dcfg.variates = {m: dcfg.variates[m] for m in dcfg.modalities}
        mods = dcfg.modalities
        variates = dcfg.variates
        num_classes = dcfg.num_classes
        if args.fold is not None:
            fc = dcfg.folds[args.fold]
            train_subj, eval_subj = fc['train_set'], fc['eval_set']
        else:
            train_subj, eval_subj = dcfg.train_set, dcfg.eval_set
        val_subj = dcfg.val_set
        input_length = ((dcfg.duration * dcfg.base_sample_rate) // args.word_length
                        if args.transform == 'sax'
                        else dcfg.duration * dcfg.base_sample_rate)
        root_dir = args.data_root or '/files1/haodong/data/processed_dalia_activity'
        sax_params = {'alphabet_size': 20, 'word_length': args.word_length}
        tds = HARDataset(root_dir, mods, train_subj, dcfg, args.transform, sax_params=sax_params)
        vds = HARDataset(root_dir, mods, val_subj, dcfg, args.transform, sax_params=sax_params)
        eds = HARDataset(root_dir, mods, eval_subj, dcfg, args.transform, sax_params=sax_params)
        ...
        val_subj = dcfg.val_set
        input_length = (dcfg.duration * dcfg.base_sample_rate) // args.word_length
        cs_dir = args.data_root or dcfg.cross_subject_dir
        sax_params = {'alphabet_size': 20, 'word_length': args.word_length}
        tds = load_pamap2_subjects(cs_dir, mods, train_subj, dcfg, args.transform, sax_params=sax_params)
        vds = load_pamap2_subjects(cs_dir, mods, val_subj, dcfg, args.transform, sax_params=sax_params)
        eds = load_pamap2_subjects(cs_dir, mods, eval_subj, dcfg, args.transform, sax_params=sax_params)
        ...
        input_length = (dcfg.duration * dcfg.base_sample_rate) // args.word_length
        sax_params = {'alphabet_size': 20, 'word_length': args.word_length}
        scenario = args.fold if args.fold is not None else 0
        root = args.data_root or '/files1/haodong/data/DSADS/'
        _src = torch.load(os.path.join(root, f'dsad_diversify_dict_scenario{scenario}_src.pt'),
                          weights_only=False)
        _trg = torch.load(os.path.join(root, f'dsad_diversify_dict_scenario{scenario}_trg.pt'),
                          weights_only=False)
        _trx, _vax, _try, _vay = train_test_split(
            _src['samples'], _src['labels'], test_size=0.2,
            random_state=2711, stratify=_src['labels'])
        tds = DSADSDataset({'samples': _trx, 'labels': _try}, mods, dcfg, args.transform, sax_params=sax_params)
        vds = DSADSDataset({'samples': _vax, 'labels': _vay}, mods, dcfg, args.transform, sax_params=sax_params)
        eds = DSADSDataset(_trg, mods, dcfg, args.transform, sax_params=sax_params)
    else:
    ...
    print(f'Modalities: {mods}  input_length={input_length}  transform={args.transform}')
    tl = DataLoader(tds, batch_size=args.batch_size, shuffle=True,
                    num_workers=args.num_workers, drop_last=True)
    vl = DataLoader(vds, batch_size=args.batch_size, shuffle=True,
                    num_workers=args.num_workers, drop_last=True)
    el = DataLoader(eds, batch_size=args.batch_size, shuffle=False,
                    num_workers=args.num_workers, drop_last=True)

    # HAR video dims default to the first modality's variate count (cfg shim parity).
    first = mods[0]
    v_dim = variates[first]
    model = _make_model(args, mods, variates, num_classes, input_length,
                        v_dim, v_dim, device)
    chmap = _channel_map(mods, variates)
    return (_preload_concat(tl, chmap), _preload_concat(vl, chmap),
            _preload_concat(el, chmap), mods, num_classes, model)


# ============================================================================
# Train / eval loop (operates on preloaded (high, low, y) batches)
# ============================================================================
    ...
    for i, (high, low, y) in enumerate(batches):
        ...
        if prefix_supervision:
            logits, prefix_logits = model(
                high, low, modality_dropout_probs=modality_dropout_probs,
            ...
            kd = sum(F.kl_div(F.log_softmax(prefix_logits[:, j], dim=-1), teach,
                              reduction='batchmean') for j in range(K)) / K
...
def build_parser():
    ...
    # ---- SAX / HAR ----
    p.add_argument('--word_length', default=2, type=int,
...
def main():
    ...
    if args.fusion_mode == 'sequential':
        variant_tag = 'seqC_depth' if args.seq_depth_flow else 'seqA'
    ...
    criterion = (FocalLoss(gamma=args.focal_gamma) if args.loss == 'focal'
