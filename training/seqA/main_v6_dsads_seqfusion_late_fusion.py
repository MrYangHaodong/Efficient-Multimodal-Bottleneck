"""V6 SEQUENTIAL-fusion training on PAMAP2 (方案 A / B-C) with the same
dynamic / grad-norm-based modality-dropout curriculum as the dyna script.

This is a copy of ``main_v6_pamap2_fusion_only_dyna_late_fusion.py`` that
...
Same backbone otherwise as `main_v6_pamap2_fusion_only.py` (no_selector=True,
opt + batched fusion).  The modality-drop schedule:

  - `main_v6_pamap2_fusion_only.py`     : uniform linear ramp; every
...
Run:
    # 方案 A, fold 0
    python main_v6_pamap2_seqfusion_late_fusion.py --fusion_mode sequential --fold=0 --cuda_pick=cuda:0
    # 方案 B/C (depth flow), fold 0
    python main_v6_pamap2_seqfusion_late_fusion.py --fusion_mode sequential --seq_depth_flow --fold=0 --cuda_pick=cuda:0
    # joint baseline, fold 0
    python main_v6_pamap2_seqfusion_late_fusion.py --fusion_mode joint --fold=0 --cuda_pick=cuda:0
    # aggregate the 3 folds of a run
    python main_v6_pamap2_seqfusion_late_fusion.py --fusion_mode sequential --aggregate
"""
...
parser = argparse.ArgumentParser(
    description='V6 fusion-only training on PAMAP2 with grad-norm-based '
...
parser.add_argument('--results_dir', default='./results_v6_seqfusion_pamap2/', type=str)
...
train_ds = DSADSDataset({'samples': _trx, 'labels': _try}, modalities, dataset_cfg, args.transform, sax_params=sax_params)
val_ds   = DSADSDataset({'samples': _vax, 'labels': _vay}, modalities, dataset_cfg, args.transform, sax_params=sax_params)
eval_ds  = DSADSDataset(_trg, modalities, dataset_cfg, args.transform, sax_params=sax_params)
