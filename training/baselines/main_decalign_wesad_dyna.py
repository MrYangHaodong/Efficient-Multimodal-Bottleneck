...
from utils.dataset_cfg import WESAD                                     # noqa: E402
...
parser.add_argument('--exp_name', default='decalign_wesad_dyna', type=str)
parser.add_argument('--results_dir',
                    default='./results_decalign_wesad_dyna/', type=str)
parser.add_argument('--dataset', default='WESAD', type=str)
...
# SAX on WESAD to match crossattn/dymo/multimodn/seqA. word_length=2 ->
# input_length=T_raw//2, matching HARDataset's SAX tokenisation.
parser.add_argument('--transform', default='sax', type=str,
                    choices=['none', 'None', 'sax'])
...
parser.add_argument('--num_prototypes', default=3, type=int,
                    help='WESAD has M=10 modalities; DecAlign multi-marginal '
                         'Sinkhorn OT is O(K^M), so K=5 (daliahar/iemocap '
                         'default) OOMs at M=10. K=3 fits at batch=64. This is '
                         'the only DecAlign-internal deviation; transform/'
                         'd_model/batch/epochs stay canonical (sax/64/64/100).')
...
parser.add_argument('--data_root',
                    default='/files1/haodong/data/WESAD/processed_wesad_activity',
                    type=str)
...
# word_length drives BOTH the SAX tokenisation granularity (HARDataset) and the
# model input_length: word_length=2 -> T=128 (default), word_length=1 -> T=256.
if transform == 'sax':
...
train_ds = HARDataset(args.data_root, modalities, train_subjects,
                      dataset_cfg, transform, sax_params=sax_params)
val_ds = HARDataset(args.data_root, modalities, val_subjects,
                    dataset_cfg, transform, sax_params=sax_params)
eval_ds = HARDataset(args.data_root, modalities, eval_subjects,
                     dataset_cfg, transform, sax_params=sax_params)
...
inner = build_decalign(
    ...
    transform=transform, alphabet_size=args.alphabet_size,
...
config = {
    ...
    'model_variant': 'decalign_wesad_dyna',
