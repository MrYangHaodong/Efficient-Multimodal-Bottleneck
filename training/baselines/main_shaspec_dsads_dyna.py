"""ShaSpec baseline training on PAMAP2 with the V6 **dyna**
(grad-norm-based asymmetric modality-dropout) curriculum.

Schedule mirrors ``main_v6_pamap2_fusion_only_dyna.py``:
    epoch < 20%        -> drop_prob = 0   (clean)
    20% <= epoch < 50% -> linear 0 -> max_drop
    epoch >= 50%       -> max_drop  (plateau)

Once drop_prob > 0, every ``--profile_update_freq`` epochs we sample
recent gradient norms (via :class:`ModalityGradientProfiler` hooked on
``model.input_projections``) and rescale per-modality drop probability.

Model: :class:`GenericShaSpecClassifier` -- generalised port of
``DaliaShaSpecClassifier`` (shared/specific encoder split + compositional
fusion + domain classifier).  Uses SAX tokens for PAMAP2.

Run:
    python main_shaspec_pamap2_dyna.py --fold=0 --cuda_pick=cuda:0
    python main_shaspec_pamap2_dyna.py --aggregate
"""
...
from multimodal_model.shaspec_baseline import (                            # noqa: E402
    GenericShaSpecClassifier, shaspec_losses,
)


# ============================ Wrapper ============================

class ShaSpecDynaWrapper(nn.Module):
    """Forwards (B,T,C_total) tensor + per-modality drop dict to
    :class:`GenericShaSpecClassifier` (which already supports per-modality
    dropout via ``modality_dropout_probs``)."""
    ...
    def forward(self, x, modality_dropout_probs=None, training=False):
        return self.model(
            x, modality_dropout_probs=modality_dropout_probs,
            training=training)
...
def train_one_epoch(loader, model, optimizer, epoch, device,
                    modality_dropout_probs, alpha, beta,
                    clip_grad=5.0, label_smoothing=0.0):
    ...
    for i, (x, y) in enumerate(loader):
        ...
        out = model(x, modality_dropout_probs=modality_dropout_probs,
                    training=True)
        total, _ = shaspec_losses(out, y, alpha=alpha, beta=beta)
        # Optional label smoothing applied to the task term.
        if label_smoothing > 0:
            task_ls = F.cross_entropy(out['logits'], y,
                                      label_smoothing=label_smoothing)
            total = total - F.cross_entropy(out['logits'], y) + task_ls
...
parser = argparse.ArgumentParser(
    description='ShaSpec on PAMAP2 with V6 grad-norm dyna modality-drop.')

parser.add_argument('--exp_name', default='shaspec_pamap2_dyna', type=str)
parser.add_argument('--results_dir',
                    default='./results_shaspec_pamap2_dyna/', type=str)
...
# ShaSpec on PAMAP2 uses SAX tokenisation (matches paper recipe).
...
# ShaSpec aux-loss weights
parser.add_argument('--alpha', default=0.1, type=float,
                    help='Shared-consistency loss weight')
parser.add_argument('--beta', default=0.02, type=float,
                    help='Specific-domain CE loss weight')
...
train_ds = DSADSDataset({'samples': _trx, 'labels': _try}, modalities, dataset_cfg, 'sax', sax_params=sax_params)
val_ds   = DSADSDataset({'samples': _vax, 'labels': _vay}, modalities, dataset_cfg, 'sax', sax_params=sax_params)
eval_ds  = DSADSDataset(_trg, modalities, dataset_cfg, 'sax', sax_params=sax_params)
...
inner = GenericShaSpecClassifier(
...
model = ShaSpecDynaWrapper(inner)
...
config = {
    ...
    'model_variant': 'shaspec_pamap2_dyna',
    'model_class': 'GenericShaSpecClassifier',
