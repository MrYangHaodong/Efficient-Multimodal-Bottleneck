"""MultiModN baseline training on WESAD with the V6 **dyna**
...
WESAD specifics mirror ``main_flexmoe_wesad_dyna.py``:
  * SAX tokenisation (``input_mode='sax'``).
  * Subject-disjoint 3-fold cross-validation (``--fold``); the WESAD
...
Run:
    python main_multimodn_wesad_dyna.py --fold=0 --cuda_pick=cuda:0 \
        --num_layers=2 --num_layers_fus=4
    python main_multimodn_wesad_dyna.py --aggregate
"""
...
parser = argparse.ArgumentParser(
    description='MultiModN on WESAD with V6 grad-norm dyna modality-drop.')

parser.add_argument('--exp_name', default='multimodn_wesad_dyna', type=str)
parser.add_argument('--results_dir',
                    default='./results_multimodn_wesad_dyna/', type=str)
...
# MultiModN on WESAD uses SAX tokenisation. Structure aligned to V6:
...
config = {
    ...
    'model_variant': 'multimodn_wesad_dyna',
