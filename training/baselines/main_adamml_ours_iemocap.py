"""AdaMML baseline (whole-sample modality selection) on IEMOCAP.

IEMOCAP variant of main_adamml_ours.py. Identical AdaMML logic (model build,
...
# ---- IEMOCAP plumbing (mirrors main_dynmm_seqA_gate_iemocap.py) ----
DATA_ROOT = '/files1/haodong/data/IEMOCAP'
SPLIT_MODE = 'cross_subject'
MODS = list(IEMOCAPDataset.ALL_MODALITIES)   # ['video','audio','text','mocap_hand','mocap_head','mocap_rotated']
M = len(MODS); NUMC = NUM_CLASSES
VAR = {m: FEATURE_DIMS[m] for m in MODS}      # per-modality in_dim -> mod_dims


# ---- loader params: read a seqA_iemocap config (max_seq_len/compression/batched) so
...
def unpack(batch):
    """IEMOCAP collate -> (high0, low0, high1, low1, ..., label[B,1]). Returns the
