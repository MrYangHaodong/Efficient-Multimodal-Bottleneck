"""CrossAttnV6Clf fusion baseline on IEMOCAP (6-modality, 4-class).

Fair model-swap counterpart of ``main_v6_iemocap_fusion_only_dyna.py``:
identical data pipeline (``IEMOCAPDataset`` via ``get_dataloader``,
...
``IEMOCAPDataset`` (downsampled when T>max_seq_len, zero-padded
...
Run:
    python script/main_crossattn_v6_iemocap.py --fold=0 --cuda_pick=cuda:0
    python script/main_crossattn_v6_iemocap.py --aggregate
"""
...
# v6_organized root for IEMOCAPDataset / train_utils.
...
class CrossAttnV6IEMOCAPDynaWrapper(nn.Module):
    """Unpack the IEMOCAPDataset batch (2*M modality tensors + label),
    ...
    IEMOCAPDataset emits them in this exact order.
    """
...
# requires a shared T across modalities; IEMOCAPDataset resamples
...
if args.max_seq_len <= 0:
    raise ValueError(
        '--max_seq_len must be > 0 for CrossAttnV6Clf on IEMOCAP '
...
cfg = _IEMOCAPCrossAttnCfg(modalities)
...
model = CrossAttnV6IEMOCAPDynaWrapper(inner, modalities, cfg.variates)
