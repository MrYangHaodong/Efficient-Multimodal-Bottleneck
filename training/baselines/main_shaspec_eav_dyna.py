"""ShaSpec baseline training on EAV with the V6 **dyna**
...
Schedule mirrors ``main_v6_eav_fusion_only_dyna.py``.
"""
...
class ShaSpecEAVDynaWrapper(nn.Module):
    """Unpacks 2M+1 EAV batch -> ``[B, T, sum(variates)]`` concat.
...
parser = argparse.ArgumentParser(
    description='ShaSpec on EAV with V6 grad-norm dyna modality-drop.')

parser.add_argument('--exp_name', default='shaspec_eav_dyna', type=str)
parser.add_argument('--results_dir',
                    default='./results_shaspec_eav_dyna/', type=str)
...
model = ShaSpecEAVDynaWrapper(inner, modalities)
...
config = {
    ...
    'model_variant': 'shaspec_eav_dyna',
