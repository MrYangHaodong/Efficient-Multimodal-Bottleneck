"""DecAlign baseline training on DaliaHAR with the V6 **dyna**
(grad-norm-based asymmetric modality-dropout) curriculum.

Same dyna schedule as ``main_v6_daliahar_fusion_only_dyna.py``:

    epoch < 20% of num_epochs           -> drop_prob = 0   (clean)
    20% <= epoch < 50%                  -> linear 0 -> max_drop
    epoch >= 50%                        -> max_drop  (plateau)

Once drop_prob > 0, every ``--profile_update_freq`` epochs we sample recent
gradient norms (via :class:`ModalityGradientProfiler` hooked on
``model.input_projections`` (alias of DecAlign's ``proj`` ModuleDict)) and
rescale per-modality drop probability proportional to its grad-norm share.

Model: ``DecAlignDSADS`` (alignment-based fusion with shared/specific
decoupling + multi-marginal Sinkhorn OT) from
``MAESTRO_ttn_robustness_old/models/decalign_dsads.py``.  Generic over any
cfg with ``modalities``, ``variates``, ``num_classes``.

Run:
    python main_decalign_daliahar_dyna.py --fold=0 --cuda_pick=cuda:0
    python main_decalign_daliahar_dyna.py --aggregate
"""
...
from utils.dataset_cfg import DSADS                                       # noqa: E402
from data.dataset_builder import DSADSDataset                             # noqa: E402
...
class DecAlignDynaWrapper(nn.Module):
    """Channel-split modality dropout on the concatenated [B,T,C_total]
    input, then forwards to DecAlign.

    DecAlign internally splits the tensor by ``cfg.variates`` and projects
    each modality with its own Conv1d.  When a modality is "dropped" we
    zero its channel slice — DecAlign still consumes all M streams but the
    masked one contributes near-zero signal to the downstream alignment
    losses and fusion paths.  This matches the semantics used by
    ``main_decalign_daliahar_unifmod.py``.
    """
    ...
    def forward(self, x, modality_dropout_probs=None, training=False):
        if training and modality_dropout_probs is not None:
            # Per-modality independent Bernoulli with at-least-1 guarantee.
            kept_idx = None
            for _ in range(8):
...
parser = argparse.ArgumentParser(
    description='DecAlign on DaliaHAR with V6 grad-norm dyna modality-drop.')

parser.add_argument('--exp_name', default='decalign_pamap2_dyna', type=str)
parser.add_argument('--results_dir',
                    default='./results_decalign_pamap2_dyna/', type=str)
...
# PAMAP2 canonical = sax_noisy (matches crossattn/dymo/seqA). word_length=1
# -> input_length = duration*rate = 512 (PAMAP2 sax_noisy keeps T). The model
# embeds the integer tokens via build_decalign(transform='sax').
parser.add_argument('--transform', default='sax_noisy', type=str,
                    choices=['none', 'None', 'sax', 'sax_noisy'])
...
# DecAlign hyperparams
parser.add_argument('--d_model', default=64, type=int)
parser.add_argument('--nhead', default=8, type=int)
parser.add_argument('--num_layers', default=3, type=int)
parser.add_argument('--num_prototypes', default=5, type=int)
...
parser.add_argument('--alpha1', default=0.1, type=float,
                    help='DecAlign decouple-loss weight')
parser.add_argument('--alpha2', default=0.01, type=float,
                    help='DecAlign alignment-loss weight (hete + homo)')
...
parser.add_argument('--data_root',
                    default='/files1/haodong/data/processed_dalia_activity',
                    type=str)
...
transform = (None if args.transform.lower() == 'none' else args.transform)
...
_src = torch.load(os.path.join(DATA_ROOT, f'dsad_diversify_dict_scenario{scenario_idx}_src.pt'), weights_only=False)
_trg = torch.load(os.path.join(DATA_ROOT, f'dsad_diversify_dict_scenario{scenario_idx}_trg.pt'), weights_only=False)
...
train_ds = DSADSDataset({'samples': _trx, 'labels': _try}, modalities, dataset_cfg, transform, sax_params=sax_params)
val_ds   = DSADSDataset({'samples': _vax, 'labels': _vay}, modalities, dataset_cfg, transform, sax_params=sax_params)
eval_ds  = DSADSDataset(_trg, modalities, dataset_cfg, transform, sax_params=sax_params)
...
inner = build_decalign(
    cfg=dataset_cfg, input_length=input_length,
    ...
    transform='sax', alphabet_size=args.alphabet_size,
    token_dim=args.token_dim, verbose=True,
)
model = DecAlignDynaWrapper(inner, modalities, dataset_cfg.variates)
...
config = {
    ...
    'model_variant': 'decalign_pamap2_dyna',
