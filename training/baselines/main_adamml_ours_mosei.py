"""AdaMML baseline (whole-sample modality selection) on MOSEI.

MOSEI variant of main_adamml_ours.py. Identical AdaMML logic (model build,
...
# ---- MOSEI plumbing (mirrors main_dynmm_seqA_gate_mosei.py) ----
DATA_ROOT = '/files1/haodong/data/CMU-MOSI/CMU-MOSEI'
SPLIT_MODE = 'random'
MODS = list(CMUMOSEIDataset.ALL_MODALITIES)   # ['video','audio','text','mocap_hand','mocap_head','mocap_rotated']
M = len(MODS); NUMC = NUM_CLASSES
VAR = {m: FEATURE_DIMS[m] for m in MODS}      # per-modality in_dim -> mod_dims


# ---- loader params: read a seqA_mosei config (max_seq_len/compression/batched) so
# loaders match what the experts/seqA were trained on (mirrors the dynmm script). ----
def _seqa_loader_cfg():
    seqa = sorted(glob.glob('../results_3p3/seqA_mosei/*'))
    for d in seqa:
...
Lc = {'max_seq_len': 50, 'time_compression_ratio': 4, 'use_batched_fusion': True}  # MOSEI fixed T=50
...
def unpack(batch):
    """MOSEI collate -> (high0, low0, high1, low1, ..., label[B,1]). Returns the
