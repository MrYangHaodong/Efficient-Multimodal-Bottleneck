"""CrossAttnV6Clf fusion baseline on MOSEI (6-modality, 4-class).

Fair model-swap counterpart of ``main_v6_mosei_fusion_only_dyna.py``:
identical data pipeline (``CMUMOSEIDataset`` via ``get_dataloader``,
...
``CMUMOSEIDataset`` (downsampled when T>max_seq_len, zero-padded
...
Run:
    python script/main_crossattn_v6_mosei.py --fold=0 --cuda_pick=cuda:0
    python script/main_crossattn_v6_mosei.py --aggregate
"""
...
# v6_organized root for CMUMOSEIDataset / train_utils.
...
class CrossAttnV6MOSEIDynaWrapper(nn.Module):
    """Unpack the CMUMOSEIDataset batch (2*M modality tensors + label),
    ...
    CMUMOSEIDataset emits them in this exact order.
    """
...
# requires a shared T across modalities; CMUMOSEIDataset resamples
...
parser.add_argument('--sparse_attn_variant', default='orig', type=str,
...
parser.add_argument('--weight_decay', default=1e-4, type=float,
                    help='AdamW weight decay (unified to 1e-4 across MOSEI models).')
...
if args.max_seq_len <= 0:
    raise ValueError(
        '--max_seq_len must be > 0 for CrossAttnV6Clf on MOSEI '
...
cfg = _MOSEICrossAttnCfg(modalities)
...
model = CrossAttnV6MOSEIDynaWrapper(inner, modalities, cfg.variates)
