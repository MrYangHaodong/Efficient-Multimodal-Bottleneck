"""MultiModN baseline training on MOSEI with the V6 **dyna**
...
MOSEI specifics mirror ``main_flexmoe_mosei_dyna.py``:
...
Run:
    python main_multimodn_mosei_dyna.py --fold=0 --cuda_pick=cuda:0 \
...
class MultiModNMOSEIDynaWrapper(nn.Module):
    """Unpacks the CMUMOSEIDataset 2M+1 batch and concatenates the high
...
parser = argparse.ArgumentParser(
    description='MultiModN on MOSEI with V6 grad-norm dyna modality-drop.')

parser.add_argument('--exp_name', default='multimodn_mosei_dyna', type=str)
parser.add_argument('--results_dir',
                    default='./results_multimodn_mosei_dyna/', type=str)
...
# MultiModN on MOSEI -- raw float features (no SAX). Structure aligned to
...
model = MultiModNMOSEIDynaWrapper(inner, modalities)
...
config = {
    ...
    'model_variant': 'multimodn_mosei_dyna',
