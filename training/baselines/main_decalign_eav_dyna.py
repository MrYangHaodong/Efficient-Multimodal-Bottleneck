"""DecAlign baseline training on EAV with the V6 **dyna**
(grad-norm-based asymmetric modality-dropout) curriculum.

EAV-specific notes vs the DaliaHAR sibling:
...
Run:
    python main_decalign_eav_dyna.py --fold=0 --cuda_pick=cuda:0
    python main_decalign_eav_dyna.py --aggregate
"""
...
class DecAlignEAVDynaWrapper(nn.Module):
    """Unpacks the EAVDataset batch (2M interleaved tensors + label),
...
parser = argparse.ArgumentParser(
    description='DecAlign on EAV with V6 grad-norm dyna modality-drop.')

parser.add_argument('--exp_name', default='decalign_eav_dyna', type=str)
parser.add_argument('--results_dir',
                    default='./results_decalign_eav_dyna/', type=str)
...
# Sequence-length (EAV loader pads/truncates every modality to this T)
...
print(f'EAV split id: {fold_id} (fold={args.fold})')
...
model = DecAlignEAVDynaWrapper(inner, modalities)
...
config = {
    ...
    'model_variant': 'decalign_eav_dyna',
