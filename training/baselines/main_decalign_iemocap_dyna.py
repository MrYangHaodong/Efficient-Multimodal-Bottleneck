"""DecAlign baseline training on IEMOCAP with the V6 **dyna**
(grad-norm-based asymmetric modality-dropout) curriculum.

IEMOCAP-specific notes vs the DaliaHAR sibling:
...
Run:
    python main_decalign_iemocap_dyna.py --fold=0 --cuda_pick=cuda:0
    python main_decalign_iemocap_dyna.py --aggregate
"""
...
class DecAlignIEMOCAPDynaWrapper(nn.Module):
    """Unpacks the IEMOCAPDataset batch (2M interleaved tensors + label),
...
parser = argparse.ArgumentParser(
    description='DecAlign on IEMOCAP with V6 grad-norm dyna modality-drop.')

parser.add_argument('--exp_name', default='decalign_iemocap_dyna', type=str)
parser.add_argument('--results_dir',
                    default='./results_decalign_iemocap_dyna/', type=str)
...
# Sequence-length (IEMOCAP loader pads/truncates every modality to this T)
...
model = DecAlignIEMOCAPDynaWrapper(inner, modalities)
...
config = {
    ...
    'model_variant': 'decalign_iemocap_dyna',
