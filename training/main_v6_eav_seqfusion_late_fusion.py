"""V6 fusion-only training on EAV with **dynamic / grad-norm-based**
modality-dropout curriculum.

EAV sibling of ``main_v6_dsads_fusion_only_dyna.py`` /
...
Differences vs DSADS / DaliaHAR:
  ...
  - ``EAVDataset`` returns a list of (high, low) tensor pairs +
...
Run:
    python main_v6_eav_fusion_only_dyna.py --fold=0 --cuda_pick=cuda:0
    python main_v6_eav_fusion_only_dyna.py --aggregate
"""
...
class _EAVV6Cfg:
    ...
    def __init__(self, modalities=None):
        ...
        # separate dims for selector vs encoder paths).  For EAV
...
class V6EAVFusionDynaWrapper(nn.Module):
    """Unpack the EAVDataset batch (12 modality tensors + label)
...
parser = argparse.ArgumentParser(
    description='V6 fusion-only training on EAV with grad-norm '
...
parser.add_argument('--results_dir',
                    default='./results_v6_seqfusion_eav/', type=str)
...
parser.add_argument('--n_fusion_distill', default=2, type=int,
...
for epoch in range(num_epochs):
    ...
    if epoch >= mod_drop_complete_epoch:
        if len(modalities) >= 2:
            ...
            # single-modality expert: LOO / rand-avg over modalities are degenerate;
            # fall back to the plain val metrics for both selection criteria.
            dv = {'accA': val_acc, 'f1A': val_f1, 'orderA': list(modalities),
                  'accB': val_acc, 'f1B': val_f1}
