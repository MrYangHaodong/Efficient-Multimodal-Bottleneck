"""CREMA-style baseline (modality-axis modular early exit) on all 6 datasets via
--dataset (iemocap / daliahar / pamap2 / dsads / eav / mosei). Concept port of CREMA:
per-modality modular adapters + modality-sequential training + modality-adaptive
early exit. MODALITY-axis early exit. See models/crema.py.

  python training/crema.py --dataset iemocap --fold 0 --results_dir ./results_crema
  python training/crema.py --dataset eav     --fold 0 --results_dir ./results_crema
"""
...
from models.crema import CREMAStyleClassifier
...
ap.add_argument('--exp_name', default='crema')
args = ap.parse_args()
...
M = model.M
print(f'[CREMA] {args.dataset} M={cfg.modalities} layers={args.num_layers} '
...
res = {'dataset': args.dataset, 'fold': args.fold, 'method': 'crema',
...
print(f'[CREMA] {args.dataset} TEST acc={ta:.4f} f1={tf:.4f} | '
